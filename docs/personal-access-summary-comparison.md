...
- **多页路径从未执行。** 测试账号三个集合的 `pagination.total` 均 ≤ 4,
  `ceil(total / 100)` 恒为 1,两版的翻页与去重逻辑都是未验证代码。需要某个集合超过
  100 条的账号才能覆盖。
- **上表的 25 次请求是推算值**,依据是 workflow 的五步定义,没有抓包确认每步各发一次
  请求。
- **`Admin` 角色的权限集未经接口验证。** 测试中的 41 项来自
  `get-global-permissions` 的有效权限结果,不是从角色名推断,也未与角色定义比对。
