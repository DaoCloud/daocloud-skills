// Code generated by lathe codegen. DO NOT EDIT.

package crane

import (
	"github.com/spf13/cobra"

	"github.com/lathe-cli/lathe/pkg/runtime"
)

const generatedSchemaVersion = 17

func Mount(root *cobra.Command) error {
	if err := runtime.AssertSchema(generatedSchemaVersion); err != nil {
		return err
	}
	return runtime.Build(root, "business-cockpit", Specs)
}

func MountFlat(root *cobra.Command) error {
	if err := runtime.AssertSchema(generatedSchemaVersion); err != nil {
		return err
	}
	return runtime.BuildFlat(root, "business-cockpit", Specs)
}

var Specs = []runtime.CommandSpec{
	{
		Group:       "BusinessOperationService",
		Use:         "get-application-agent-count",
		Short:       "GetApplicationAgentCount returns the estimated application / agent count KPI.",
		OperationID: "BusinessOperationService_GetApplicationAgentCount",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-operation/application-agent-count",
		Params: []runtime.ParamSpec{
			{Name: "startTime", Flag: "start-time", In: "query", GoType: "string", Help: "Start time for the statistics period (UTC, inclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "endTime", Flag: "end-time", In: "query", GoType: "string", Help: "End time for the statistics period (UTC, exclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "limit", Flag: "limit", In: "query", GoType: "int64", Help: "Limit for top-N style queries. (query, int32)", Required: false, Format: "int32"},
			{Name: "period", Flag: "period", In: "query", GoType: "string", Help: "Preset period key, such as thisMonth / lastMonth / thisQuarter / thisYear. (query)", Required: false},
			{Name: "model", Flag: "model", In: "query", GoType: "string", Help: "Optional model keyword filter. (query)", Required: false},
		},
		Output: runtime.OutputHints{Pagination: &runtime.PaginationHint{Strategy: "cursor", LimitParam: "limit"}},
	},
	{
		Group:       "BusinessOperationService",
		Use:         "get-application-type-distribution",
		Short:       "GetApplicationTypeDistribution returns the estimated application type distribution panel.",
		OperationID: "BusinessOperationService_GetApplicationTypeDistribution",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-operation/application-type-distribution",
		Params: []runtime.ParamSpec{
			{Name: "startTime", Flag: "start-time", In: "query", GoType: "string", Help: "Start time for the statistics period (UTC, inclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "endTime", Flag: "end-time", In: "query", GoType: "string", Help: "End time for the statistics period (UTC, exclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "limit", Flag: "limit", In: "query", GoType: "int64", Help: "Limit for top-N style queries. (query, int32)", Required: false, Format: "int32"},
			{Name: "period", Flag: "period", In: "query", GoType: "string", Help: "Preset period key, such as thisMonth / lastMonth / thisQuarter / thisYear. (query)", Required: false},
			{Name: "model", Flag: "model", In: "query", GoType: "string", Help: "Optional model keyword filter. (query)", Required: false},
		},
		Output: runtime.OutputHints{ListPath: "items", DefaultColumns: []string{"name", "cost", "displayValue", "rank", "revenue", "userName"}, Pagination: &runtime.PaginationHint{Strategy: "cursor", LimitParam: "limit"}},
	},
	{
		Group:       "BusinessOperationService",
		Use:         "get-current-period-business-overview",
		Short:       "GetCurrentPeriodBusinessOverview returns the WS current-period business overview.",
		OperationID: "BusinessOperationService_GetCurrentPeriodBusinessOverview",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-operation/current-period-business-overview",
		Params: []runtime.ParamSpec{
			{Name: "period", Flag: "period", In: "query", GoType: "string", Help: "period (query, one of: BUSINESS_OVERVIEW_PERIOD_UNSPECIFIED|BUSINESS_OVERVIEW_PERIOD_THIS_MONTH|BUSINESS_OVERVIEW_PERIOD_THIS_WEEK|BUSINESS_OVERVIEW_PERIOD_TODAY)", Required: false, Default: "BUSINESS_OVERVIEW_PERIOD_UNSPECIFIED", Enum: []string{"BUSINESS_OVERVIEW_PERIOD_UNSPECIFIED", "BUSINESS_OVERVIEW_PERIOD_THIS_MONTH", "BUSINESS_OVERVIEW_PERIOD_THIS_WEEK", "BUSINESS_OVERVIEW_PERIOD_TODAY"}},
		},
		Output: runtime.OutputHints{ListPath: "activeUserTrend", DefaultColumns: []string{"activeUserCount", "date", "totalUserCount"}},
	},
	{
		Group:       "BusinessOperationService",
		Use:         "get-department-application-model-gpu-flow",
		Short:       "GetDepartmentApplicationModelGpuFlow returns the department -> application -> model -> GPU flow panel.",
		OperationID: "BusinessOperationService_GetDepartmentApplicationModelGpuFlow",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-operation/department-application-model-gpu-flow",
		Params: []runtime.ParamSpec{
			{Name: "startTime", Flag: "start-time", In: "query", GoType: "string", Help: "Start time for the statistics period (UTC, inclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "endTime", Flag: "end-time", In: "query", GoType: "string", Help: "End time for the statistics period (UTC, exclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "limit", Flag: "limit", In: "query", GoType: "int64", Help: "Limit for top-N style queries. (query, int32)", Required: false, Format: "int32"},
			{Name: "period", Flag: "period", In: "query", GoType: "string", Help: "Preset period key, such as thisMonth / lastMonth / thisQuarter / thisYear. (query)", Required: false},
			{Name: "model", Flag: "model", In: "query", GoType: "string", Help: "Optional model keyword filter. (query)", Required: false},
		},
		Output: runtime.OutputHints{Pagination: &runtime.PaginationHint{Strategy: "cursor", LimitParam: "limit"}},
	},
	{
		Group:       "BusinessOperationService",
		Use:         "get-department-budget-usage-rate",
		Short:       "GetDepartmentBudgetUsageRate returns the department budget usage rate KPI.",
		OperationID: "BusinessOperationService_GetDepartmentBudgetUsageRate",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-operation/department-budget-usage-rate",
		Params: []runtime.ParamSpec{
			{Name: "startTime", Flag: "start-time", In: "query", GoType: "string", Help: "Start time for the statistics period (UTC, inclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "endTime", Flag: "end-time", In: "query", GoType: "string", Help: "End time for the statistics period (UTC, exclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "limit", Flag: "limit", In: "query", GoType: "int64", Help: "Limit for top-N style queries. (query, int32)", Required: false, Format: "int32"},
			{Name: "period", Flag: "period", In: "query", GoType: "string", Help: "Preset period key, such as thisMonth / lastMonth / thisQuarter / thisYear. (query)", Required: false},
			{Name: "model", Flag: "model", In: "query", GoType: "string", Help: "Optional model keyword filter. (query)", Required: false},
		},
		Output: runtime.OutputHints{Pagination: &runtime.PaginationHint{Strategy: "cursor", LimitParam: "limit"}},
	},
	{
		Group:       "BusinessOperationService",
		Use:         "get-department-monthly-allocated-cost",
		Short:       "GetDepartmentMonthlyAllocatedCost returns the monthly allocated cost KPI.",
		OperationID: "BusinessOperationService_GetDepartmentMonthlyAllocatedCost",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-operation/department-monthly-allocated-cost",
		Params: []runtime.ParamSpec{
			{Name: "startTime", Flag: "start-time", In: "query", GoType: "string", Help: "Start time for the statistics period (UTC, inclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "endTime", Flag: "end-time", In: "query", GoType: "string", Help: "End time for the statistics period (UTC, exclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "limit", Flag: "limit", In: "query", GoType: "int64", Help: "Limit for top-N style queries. (query, int32)", Required: false, Format: "int32"},
			{Name: "period", Flag: "period", In: "query", GoType: "string", Help: "Preset period key, such as thisMonth / lastMonth / thisQuarter / thisYear. (query)", Required: false},
			{Name: "model", Flag: "model", In: "query", GoType: "string", Help: "Optional model keyword filter. (query)", Required: false},
		},
		Output: runtime.OutputHints{Pagination: &runtime.PaginationHint{Strategy: "cursor", LimitParam: "limit"}},
	},
	{
		Group:       "BusinessOperationService",
		Use:         "get-department-quota-consumption",
		Short:       "GetDepartmentQuotaConsumption returns the current WS workspace quota snapshot.",
		OperationID: "BusinessOperationService_GetDepartmentQuotaConsumption",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-operation/department-quota-consumption",
		Params: []runtime.ParamSpec{
			{Name: "limit", Flag: "limit", In: "query", GoType: "int64", Help: "limit (query, int32)", Required: false, Format: "int32"},
		},
		Output: runtime.OutputHints{ListPath: "items", DefaultColumns: []string{"departmentName", "quotaTokens", "unlimitedQuota", "usagePercent", "usedTokens", "workspace"}, Pagination: &runtime.PaginationHint{Strategy: "cursor", LimitParam: "limit"}},
	},
	{
		Group:       "BusinessOperationService",
		Use:         "get-department-unit-business-cost",
		Short:       "GetDepartmentUnitBusinessCost returns the estimated unit business cost KPI.",
		OperationID: "BusinessOperationService_GetDepartmentUnitBusinessCost",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-operation/department-unit-business-cost",
		Params: []runtime.ParamSpec{
			{Name: "startTime", Flag: "start-time", In: "query", GoType: "string", Help: "Start time for the statistics period (UTC, inclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "endTime", Flag: "end-time", In: "query", GoType: "string", Help: "End time for the statistics period (UTC, exclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "limit", Flag: "limit", In: "query", GoType: "int64", Help: "Limit for top-N style queries. (query, int32)", Required: false, Format: "int32"},
			{Name: "period", Flag: "period", In: "query", GoType: "string", Help: "Preset period key, such as thisMonth / lastMonth / thisQuarter / thisYear. (query)", Required: false},
			{Name: "model", Flag: "model", In: "query", GoType: "string", Help: "Optional model keyword filter. (query)", Required: false},
		},
		Output: runtime.OutputHints{Pagination: &runtime.PaginationHint{Strategy: "cursor", LimitParam: "limit"}},
	},
	{
		Group:       "BusinessOperationService",
		Use:         "get-department-unit-business-cost-comparison",
		Short:       "GetDepartmentUnitBusinessCostComparison returns the unit business cost comparison panel.",
		OperationID: "BusinessOperationService_GetDepartmentUnitBusinessCostComparison",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-operation/department-unit-business-cost-comparison",
		Params: []runtime.ParamSpec{
			{Name: "startTime", Flag: "start-time", In: "query", GoType: "string", Help: "Start time for the statistics period (UTC, inclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "endTime", Flag: "end-time", In: "query", GoType: "string", Help: "End time for the statistics period (UTC, exclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "limit", Flag: "limit", In: "query", GoType: "int64", Help: "Limit for top-N style queries. (query, int32)", Required: false, Format: "int32"},
			{Name: "period", Flag: "period", In: "query", GoType: "string", Help: "Preset period key, such as thisMonth / lastMonth / thisQuarter / thisYear. (query)", Required: false},
			{Name: "model", Flag: "model", In: "query", GoType: "string", Help: "Optional model keyword filter. (query)", Required: false},
		},
		Output: runtime.OutputHints{ListPath: "items", DefaultColumns: []string{"name", "cost", "displayValue", "rank", "revenue", "userName"}, Pagination: &runtime.PaginationHint{Strategy: "cursor", LimitParam: "limit"}},
	},
	{
		Group:       "BusinessOperationService",
		Use:         "get-department-value-quadrant",
		Short:       "GetDepartmentValueQuadrant returns department token consumption and active-user points.",
		OperationID: "BusinessOperationService_GetDepartmentValueQuadrant",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-operation/department-value-quadrant",
		Params: []runtime.ParamSpec{
			{Name: "startTime", Flag: "start-time", In: "query", GoType: "string", Help: "Start time for the statistics period (UTC, inclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "endTime", Flag: "end-time", In: "query", GoType: "string", Help: "End time for the statistics period (UTC, exclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "limit", Flag: "limit", In: "query", GoType: "int64", Help: "Limit for top-N style queries. (query, int32)", Required: false, Format: "int32"},
			{Name: "period", Flag: "period", In: "query", GoType: "string", Help: "Preset period key, such as thisMonth / lastMonth / thisQuarter / thisYear. (query)", Required: false},
			{Name: "model", Flag: "model", In: "query", GoType: "string", Help: "Optional model keyword filter. (query)", Required: false},
		},
		Output: runtime.OutputHints{ListPath: "points", DefaultColumns: []string{"activeUsers", "departmentId", "departmentName", "totalTokens"}, Pagination: &runtime.PaginationHint{Strategy: "cursor", LimitParam: "limit"}},
	},
	{
		Group:       "BusinessOperationService",
		Use:         "get-monthly-arpu",
		Short:       "GetMonthlyARPU returns the average revenue per active tenant in the given window.",
		OperationID: "BusinessOperationService_GetMonthlyARPU",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-operation/monthly-arpu",
		Params: []runtime.ParamSpec{
			{Name: "startTime", Flag: "start-time", In: "query", GoType: "string", Help: "Start time for the statistics period (UTC, inclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "endTime", Flag: "end-time", In: "query", GoType: "string", Help: "End time for the statistics period (UTC, exclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "limit", Flag: "limit", In: "query", GoType: "int64", Help: "Limit for top-N style queries. (query, int32)", Required: false, Format: "int32"},
			{Name: "period", Flag: "period", In: "query", GoType: "string", Help: "Preset period key, such as thisMonth / lastMonth / thisQuarter / thisYear. (query)", Required: false},
			{Name: "model", Flag: "model", In: "query", GoType: "string", Help: "Optional model keyword filter. (query)", Required: false},
		},
		Output: runtime.OutputHints{Pagination: &runtime.PaginationHint{Strategy: "cursor", LimitParam: "limit"}},
	},
	{
		Group:       "BusinessOperationService",
		Use:         "get-monthly-revenue",
		Short:       "GetMonthlyRevenue returns bill revenue in the given window.",
		OperationID: "BusinessOperationService_GetMonthlyRevenue",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-operation/monthly-revenue",
		Params: []runtime.ParamSpec{
			{Name: "startTime", Flag: "start-time", In: "query", GoType: "string", Help: "Start time for the statistics period (UTC, inclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "endTime", Flag: "end-time", In: "query", GoType: "string", Help: "End time for the statistics period (UTC, exclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "limit", Flag: "limit", In: "query", GoType: "int64", Help: "Limit for top-N style queries. (query, int32)", Required: false, Format: "int32"},
			{Name: "period", Flag: "period", In: "query", GoType: "string", Help: "Preset period key, such as thisMonth / lastMonth / thisQuarter / thisYear. (query)", Required: false},
			{Name: "model", Flag: "model", In: "query", GoType: "string", Help: "Optional model keyword filter. (query)", Required: false},
		},
		Output: runtime.OutputHints{Pagination: &runtime.PaginationHint{Strategy: "cursor", LimitParam: "limit"}},
	},
	{
		Group:       "BusinessOperationService",
		Use:         "get-monthly-token-consumption",
		Short:       "GetMonthlyTokenConsumption returns token consumption metrics in the given window.",
		OperationID: "BusinessOperationService_GetMonthlyTokenConsumption",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-operation/monthly-token-consumption",
		Params: []runtime.ParamSpec{
			{Name: "startTime", Flag: "start-time", In: "query", GoType: "string", Help: "Start time for the statistics period (UTC, inclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "endTime", Flag: "end-time", In: "query", GoType: "string", Help: "End time for the statistics period (UTC, exclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "limit", Flag: "limit", In: "query", GoType: "int64", Help: "Limit for top-N style queries. (query, int32)", Required: false, Format: "int32"},
			{Name: "period", Flag: "period", In: "query", GoType: "string", Help: "Preset period key, such as thisMonth / lastMonth / thisQuarter / thisYear. (query)", Required: false},
			{Name: "model", Flag: "model", In: "query", GoType: "string", Help: "Optional model keyword filter. (query)", Required: false},
		},
		Output: runtime.OutputHints{Pagination: &runtime.PaginationHint{Strategy: "cursor", LimitParam: "limit"}},
	},
	{
		Group:       "BusinessOperationService",
		Use:         "get-tenant-growth-trend-top5",
		Short:       "GetTenantGrowthTrendTop5 returns the top-5 tenant growth trend panel.",
		OperationID: "BusinessOperationService_GetTenantGrowthTrendTop5",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-operation/tenant-growth-trend-top5",
		Params: []runtime.ParamSpec{
			{Name: "startTime", Flag: "start-time", In: "query", GoType: "string", Help: "Start time for the statistics period (UTC, inclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "endTime", Flag: "end-time", In: "query", GoType: "string", Help: "End time for the statistics period (UTC, exclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "limit", Flag: "limit", In: "query", GoType: "int64", Help: "Limit for top-N style queries. (query, int32)", Required: false, Format: "int32"},
			{Name: "period", Flag: "period", In: "query", GoType: "string", Help: "Preset period key, such as thisMonth / lastMonth / thisQuarter / thisYear. (query)", Required: false},
			{Name: "model", Flag: "model", In: "query", GoType: "string", Help: "Optional model keyword filter. (query)", Required: false},
		},
		Output: runtime.OutputHints{Pagination: &runtime.PaginationHint{Strategy: "cursor", LimitParam: "limit"}},
	},
	{
		Group:       "BusinessOperationService",
		Use:         "get-tenant-model-gpu-token-flow",
		Short:       "GetTenantModelGpuTokenFlow returns the tenant -> model -> GPU -> token flow panel.",
		OperationID: "BusinessOperationService_GetTenantModelGpuTokenFlow",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-operation/tenant-model-gpu-token-flow",
		Params: []runtime.ParamSpec{
			{Name: "startTime", Flag: "start-time", In: "query", GoType: "string", Help: "Start time for the statistics period (UTC, inclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "endTime", Flag: "end-time", In: "query", GoType: "string", Help: "End time for the statistics period (UTC, exclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "limit", Flag: "limit", In: "query", GoType: "int64", Help: "Limit for top-N style queries. (query, int32)", Required: false, Format: "int32"},
			{Name: "period", Flag: "period", In: "query", GoType: "string", Help: "Preset period key, such as thisMonth / lastMonth / thisQuarter / thisYear. (query)", Required: false},
			{Name: "model", Flag: "model", In: "query", GoType: "string", Help: "Optional model keyword filter. (query)", Required: false},
		},
		Output: runtime.OutputHints{Pagination: &runtime.PaginationHint{Strategy: "cursor", LimitParam: "limit"}},
	},
	{
		Group:       "BusinessOperationService",
		Use:         "get-tenant-value-quadrant",
		Short:       "GetTenantValueQuadrant returns the tenant token vs revenue quadrant panel.",
		OperationID: "BusinessOperationService_GetTenantValueQuadrant",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-operation/tenant-value-quadrant",
		Params: []runtime.ParamSpec{
			{Name: "startTime", Flag: "start-time", In: "query", GoType: "string", Help: "Start time for the statistics period (UTC, inclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "endTime", Flag: "end-time", In: "query", GoType: "string", Help: "End time for the statistics period (UTC, exclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "limit", Flag: "limit", In: "query", GoType: "int64", Help: "Limit for top-N style queries. (query, int32)", Required: false, Format: "int32"},
			{Name: "period", Flag: "period", In: "query", GoType: "string", Help: "Preset period key, such as thisMonth / lastMonth / thisQuarter / thisYear. (query)", Required: false},
			{Name: "model", Flag: "model", In: "query", GoType: "string", Help: "Optional model keyword filter. (query)", Required: false},
		},
		Output: runtime.OutputHints{ListPath: "points", DefaultColumns: []string{"name", "size", "x", "y"}, Pagination: &runtime.PaginationHint{Strategy: "cursor", LimitParam: "limit"}},
	},
	{
		Group:       "BusinessOperationService",
		Use:         "list-active-departments",
		Short:       "ListActiveDepartments returns the count of active departments.",
		OperationID: "BusinessOperationService_ListActiveDepartments",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-operation/active-departments",
		Params: []runtime.ParamSpec{
			{Name: "startTime", Flag: "start-time", In: "query", GoType: "string", Help: "Start time for the statistics period (UTC, inclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "endTime", Flag: "end-time", In: "query", GoType: "string", Help: "End time for the statistics period (UTC, exclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "limit", Flag: "limit", In: "query", GoType: "int64", Help: "Limit for top-N style queries. (query, int32)", Required: false, Format: "int32"},
			{Name: "period", Flag: "period", In: "query", GoType: "string", Help: "Preset period key, such as thisMonth / lastMonth / thisQuarter / thisYear. (query)", Required: false},
			{Name: "model", Flag: "model", In: "query", GoType: "string", Help: "Optional model keyword filter. (query)", Required: false},
		},
		Output: runtime.OutputHints{Pagination: &runtime.PaginationHint{Strategy: "cursor", LimitParam: "limit"}},
	},
	{
		Group:       "BusinessOperationService",
		Use:         "list-active-tenants",
		Short:       "ListActiveTenants returns the count of active tenants with token consumption.",
		OperationID: "BusinessOperationService_ListActiveTenants",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-operation/active-tenants",
		Params: []runtime.ParamSpec{
			{Name: "startTime", Flag: "start-time", In: "query", GoType: "string", Help: "Start time for the statistics period (UTC). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "endTime", Flag: "end-time", In: "query", GoType: "string", Help: "End time for the statistics period (UTC, exclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "period", Flag: "period", In: "query", GoType: "string", Help: "Preset period key, such as thisMonth / lastMonth / thisQuarter / thisYear. (query)", Required: false},
			{Name: "model", Flag: "model", In: "query", GoType: "string", Help: "Optional model keyword filter. (query)", Required: false},
		},
	},
	{
		Group:       "BusinessOperationService",
		Use:         "list-business-models",
		Short:       "ListBusinessModels returns the model options for the business dashboard filter.",
		OperationID: "BusinessOperationService_ListBusinessModels",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-operation/models",
		Params: []runtime.ParamSpec{
			{Name: "startTime", Flag: "start-time", In: "query", GoType: "string", Help: "Start time for the statistics period (UTC, inclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "endTime", Flag: "end-time", In: "query", GoType: "string", Help: "End time for the statistics period (UTC, exclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "limit", Flag: "limit", In: "query", GoType: "int64", Help: "Limit for top-N style queries. (query, int32)", Required: false, Format: "int32"},
			{Name: "period", Flag: "period", In: "query", GoType: "string", Help: "Preset period key, such as thisMonth / lastMonth / thisQuarter / thisYear. (query)", Required: false},
			{Name: "model", Flag: "model", In: "query", GoType: "string", Help: "Optional model keyword filter. (query)", Required: false},
		},
		Output: runtime.OutputHints{ListPath: "models", Pagination: &runtime.PaginationHint{Strategy: "cursor", LimitParam: "limit"}},
	},
	{
		Group:       "BusinessOperationService",
		Use:         "list-business-operation-suggestions",
		Short:       "ListBusinessOperationSuggestions returns the aggregated tenant and department suggestions panel.",
		OperationID: "BusinessOperationService_ListBusinessOperationSuggestions",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-operation/suggestions",
		Params: []runtime.ParamSpec{
			{Name: "startTime", Flag: "start-time", In: "query", GoType: "string", Help: "Start time for the statistics period (UTC, inclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "endTime", Flag: "end-time", In: "query", GoType: "string", Help: "End time for the statistics period (UTC, exclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "limit", Flag: "limit", In: "query", GoType: "int64", Help: "Limit for top-N style queries. (query, int32)", Required: false, Format: "int32"},
			{Name: "period", Flag: "period", In: "query", GoType: "string", Help: "Preset period key, such as thisMonth / lastMonth / thisQuarter / thisYear. (query)", Required: false},
			{Name: "model", Flag: "model", In: "query", GoType: "string", Help: "Optional model keyword filter. (query)", Required: false},
		},
		Output: runtime.OutputHints{ListPath: "items", DefaultColumns: []string{"type", "content"}, Pagination: &runtime.PaginationHint{Strategy: "cursor", LimitParam: "limit"}},
	},
	{
		Group:       "BusinessOperationService",
		Use:         "list-department-token-consumption-top",
		Short:       "ListDepartmentTokenConsumptionTop returns the top department token consumers.",
		OperationID: "BusinessOperationService_ListDepartmentTokenConsumptionTop",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-operation/department-token-consumption-top",
		Params: []runtime.ParamSpec{
			{Name: "startTime", Flag: "start-time", In: "query", GoType: "string", Help: "Start time for the statistics period (UTC, inclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "endTime", Flag: "end-time", In: "query", GoType: "string", Help: "End time for the statistics period (UTC, exclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "limit", Flag: "limit", In: "query", GoType: "int64", Help: "Limit for top-N style queries. (query, int32)", Required: false, Format: "int32"},
			{Name: "period", Flag: "period", In: "query", GoType: "string", Help: "Preset period key, such as thisMonth / lastMonth / thisQuarter / thisYear. (query)", Required: false},
			{Name: "model", Flag: "model", In: "query", GoType: "string", Help: "Optional model keyword filter. (query)", Required: false},
		},
		Output: runtime.OutputHints{ListPath: "items", DefaultColumns: []string{"department", "rank", "totalTokens", "userName"}, Pagination: &runtime.PaginationHint{Strategy: "cursor", LimitParam: "limit"}},
	},
	{
		Group:       "BusinessOperationService",
		Use:         "list-tenant-risk-objects",
		Short:       "ListTenantRiskObjects returns the tenant risk object ranking panel.",
		OperationID: "BusinessOperationService_ListTenantRiskObjects",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-operation/tenant-risk-objects",
		Params: []runtime.ParamSpec{
			{Name: "startTime", Flag: "start-time", In: "query", GoType: "string", Help: "Start time for the statistics period (UTC, inclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "endTime", Flag: "end-time", In: "query", GoType: "string", Help: "End time for the statistics period (UTC, exclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "limit", Flag: "limit", In: "query", GoType: "int64", Help: "Limit for top-N style queries. (query, int32)", Required: false, Format: "int32"},
			{Name: "period", Flag: "period", In: "query", GoType: "string", Help: "Preset period key, such as thisMonth / lastMonth / thisQuarter / thisYear. (query)", Required: false},
			{Name: "model", Flag: "model", In: "query", GoType: "string", Help: "Optional model keyword filter. (query)", Required: false},
		},
		Output: runtime.OutputHints{ListPath: "items", DefaultColumns: []string{"action", "detail", "level", "riskType", "tenant", "userName"}, Pagination: &runtime.PaginationHint{Strategy: "cursor", LimitParam: "limit"}},
	},
	{
		Group:       "BusinessOperationService",
		Use:         "list-tenant-token-consumption-top",
		Short:       "ListTenantTokenConsumptionTop returns the top tenant token consumers.",
		OperationID: "BusinessOperationService_ListTenantTokenConsumptionTop",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-operation/tenant-token-consumption-top",
		Params: []runtime.ParamSpec{
			{Name: "startTime", Flag: "start-time", In: "query", GoType: "string", Help: "Start time for the statistics period (UTC, inclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "endTime", Flag: "end-time", In: "query", GoType: "string", Help: "End time for the statistics period (UTC, exclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "limit", Flag: "limit", In: "query", GoType: "int64", Help: "Limit for top-N style queries. (query, int32)", Required: false, Format: "int32"},
			{Name: "period", Flag: "period", In: "query", GoType: "string", Help: "Preset period key, such as thisMonth / lastMonth / thisQuarter / thisYear. (query)", Required: false},
			{Name: "model", Flag: "model", In: "query", GoType: "string", Help: "Optional model keyword filter. (query)", Required: false},
		},
		Output: runtime.OutputHints{ListPath: "items", DefaultColumns: []string{"rank", "tenantId", "totalTokens", "userName"}, Pagination: &runtime.PaginationHint{Strategy: "cursor", LimitParam: "limit"}},
	},
	{
		Group:       "BusinessValueService",
		Use:         "get-api-key-count",
		Short:       "GetApiKeyCount returns the count of distinct API keys used in the given time window.",
		OperationID: "BusinessValueService_GetApiKeyCount",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-value/api-key-count",
		Params: []runtime.ParamSpec{
			{Name: "startTime", Flag: "start-time", In: "query", GoType: "string", Help: "Start of the query window (UTC, inclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "endTime", Flag: "end-time", In: "query", GoType: "string", Help: "End of the query window (UTC, exclusive). (query, date-time)", Required: false, Format: "date-time"},
		},
	},
	{
		Group:       "BusinessValueService",
		Use:         "get-capacity-bottleneck-forecast",
		Short:       "GetCapacityBottleneckForecast returns the predicted number of days until",
		OperationID: "BusinessValueService_GetCapacityBottleneckForecast",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-value/capacity-bottleneck-forecast",
	},
	{
		Group:       "BusinessValueService",
		Use:         "get-capacity-funnel",
		Short:       "GetCapacityFunnel returns billed token totals and high-margin model token totals.",
		OperationID: "BusinessValueService_GetCapacityFunnel",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-value/capacity-funnel",
		Params: []runtime.ParamSpec{
			{Name: "timeRange", Flag: "time-range", In: "query", GoType: "string", Help: "Time range filter: \"today\", \"this-week\", \"this-month\", \"this-quarter\". (query)", Required: false},
		},
	},
	{
		Group:       "BusinessValueService",
		Use:         "get-consumption-revenue-cost-summary",
		Short:       "GetConsumptionRevenueCostSummary returns global current and previous period totals.",
		OperationID: "BusinessValueService_GetConsumptionRevenueCostSummary",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-value/consumption-revenue-cost-summary",
		Params: []runtime.ParamSpec{
			{Name: "timeRange", Flag: "time-range", In: "query", GoType: "string", Help: "One of \"today\", \"this-week\", \"this-month\", or \"this-quarter\". (query)", Required: false},
		},
	},
	{
		Group:       "BusinessValueService",
		Use:         "get-cumulative-token",
		Short:       "GetCumulativeToken returns cumulative token metrics for an explicit time window.",
		OperationID: "BusinessValueService_GetCumulativeToken",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-value/cumulative-token",
		Params: []runtime.ParamSpec{
			{Name: "startTime", Flag: "start-time", In: "query", GoType: "string", Help: "Inclusive range start. (query, date-time)", Required: false, Format: "date-time"},
			{Name: "endTime", Flag: "end-time", In: "query", GoType: "string", Help: "Exclusive range end. (query, date-time)", Required: false, Format: "date-time"},
		},
	},
	{
		Group:       "BusinessValueService",
		Use:         "get-cumulative-token-history",
		Short:       "GetCumulativeTokenHistory returns token totals for each bucket in an explicit time window.",
		OperationID: "BusinessValueService_GetCumulativeTokenHistory",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-value/cumulative-token-history",
		Params: []runtime.ParamSpec{
			{Name: "startTime", Flag: "start-time", In: "query", GoType: "string", Help: "Inclusive range start. (query, date-time)", Required: false, Format: "date-time"},
			{Name: "endTime", Flag: "end-time", In: "query", GoType: "string", Help: "Exclusive range end. (query, date-time)", Required: false, Format: "date-time"},
			{Name: "span", Flag: "span", In: "query", GoType: "string", Help: "Bucket span used to aggregate history points. (query, one of: CUMULATIVE_TOKEN_HISTORY_SPAN_UNSPECIFIED|CUMULATIVE_TOKEN_HISTORY_SPAN_HOUR|CUMULATIVE_TOKEN_HISTORY_SPAN_DAY)", Required: false, Default: "CUMULATIVE_TOKEN_HISTORY_SPAN_UNSPECIFIED", Enum: []string{"CUMULATIVE_TOKEN_HISTORY_SPAN_UNSPECIFIED", "CUMULATIVE_TOKEN_HISTORY_SPAN_HOUR", "CUMULATIVE_TOKEN_HISTORY_SPAN_DAY"}},
		},
		Output: runtime.OutputHints{ListPath: "points", DefaultColumns: []string{"time", "totalTokens"}},
	},
	{
		Group:       "BusinessValueService",
		Use:         "get-customer-concentration-thresholds",
		Short:       "GetCustomerConcentrationThresholds returns the warn/risk Top1 and Top3",
		OperationID: "BusinessValueService_GetCustomerConcentrationThresholds",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-value/customer-concentration-thresholds",
	},
	{
		Group:       "BusinessValueService",
		Use:         "get-department-token-usage",
		Short:       "GetDepartmentTokenUsage returns per-department token usage and budget for the given time window.",
		OperationID: "BusinessValueService_GetDepartmentTokenUsage",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-value/department-token-usage",
		Params: []runtime.ParamSpec{
			{Name: "startTime", Flag: "start-time", In: "query", GoType: "string", Help: "Start of the query window (UTC, inclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "endTime", Flag: "end-time", In: "query", GoType: "string", Help: "End of the query window (UTC, exclusive). (query, date-time)", Required: false, Format: "date-time"},
		},
		Output: runtime.OutputHints{ListPath: "items", DefaultColumns: []string{"budgetTokenTotal", "departmentName", "tokenTotal"}},
	},
	{
		Group:       "BusinessValueService",
		Use:         "get-gross-profit-attribution-config",
		Short:       "GetGrossProfitAttributionConfig returns the gross profit attribution configuration",
		OperationID: "BusinessValueService_GetGrossProfitAttributionConfig",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/businessvalue/gross-profit-attribution-config",
	},
	{
		Group:       "BusinessValueService",
		Use:         "get-model-contribution",
		Short:       "GetModelContribution returns model-level revenue and token totals for an explicit UTC time window.",
		OperationID: "BusinessValueService_GetModelContribution",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-value/model-contribution",
		Params: []runtime.ParamSpec{
			{Name: "startTime", Flag: "start-time", In: "query", GoType: "string", Help: "Start of the query window (UTC, inclusive). (query, date-time)", Required: false, Format: "date-time"},
			{Name: "endTime", Flag: "end-time", In: "query", GoType: "string", Help: "End of the query window (UTC, exclusive). (query, date-time)", Required: false, Format: "date-time"},
		},
		Output: runtime.OutputHints{ListPath: "items", DefaultColumns: []string{"modelName", "revenueAmount", "tokenTotal"}},
	},
	{
		Group:       "BusinessValueService",
		Use:         "get-month-end-forecast-metrics",
		Short:       "GetMonthEndForecastMetrics returns month-end revenue and gross profit forecast metrics.",
		OperationID: "BusinessValueService_GetMonthEndForecastMetrics",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-value/month-end-forecast",
	},
	{
		Group:       "BusinessValueService",
		Use:         "get-period-cost",
		Short:       "GetPeriodCost returns cost breakdown for an explicit time window.",
		OperationID: "BusinessValueService_GetPeriodCost",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-value/period-cost",
		Params: []runtime.ParamSpec{
			{Name: "startTime", Flag: "start-time", In: "query", GoType: "string", Help: "Inclusive range start. (query, date-time)", Required: false, Format: "date-time"},
			{Name: "endTime", Flag: "end-time", In: "query", GoType: "string", Help: "Exclusive range end. (query, date-time)", Required: false, Format: "date-time"},
		},
	},
	{
		Group:       "BusinessValueService",
		Use:         "get-period-revenue-cost-trend",
		Short:       "GetPeriodRevenueCostTrend returns period-aware revenue and cost trend points.",
		OperationID: "BusinessValueService_GetPeriodRevenueCostTrend",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-value/period-revenue-cost-trend",
		Params: []runtime.ParamSpec{
			{Name: "timeRange", Flag: "time-range", In: "query", GoType: "string", Help: "Required UTC natural dashboard time range. (query, required, one of: DASHBOARD_TIME_RANGE_UNSPECIFIED|DASHBOARD_TIME_RANGE_TODAY|DASHBOARD_TIME_RANGE_THIS_WEEK|DASHBOARD_TIME_RANGE_THIS_MONTH|DASHBOARD_TIME_RANGE_THIS_QUARTER)", Required: true, Default: "DASHBOARD_TIME_RANGE_UNSPECIFIED", Enum: []string{"DASHBOARD_TIME_RANGE_UNSPECIFIED", "DASHBOARD_TIME_RANGE_TODAY", "DASHBOARD_TIME_RANGE_THIS_WEEK", "DASHBOARD_TIME_RANGE_THIS_MONTH", "DASHBOARD_TIME_RANGE_THIS_QUARTER"}},
		},
		Output: runtime.OutputHints{ListPath: "points", DefaultColumns: []string{"cost", "revenue", "time"}},
	},
	{
		Group:       "BusinessValueService",
		Use:         "get-period-token-trend",
		Short:       "GetPeriodTokenTrend returns period-aware token output trend points.",
		OperationID: "BusinessValueService_GetPeriodTokenTrend",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-value/period-token-trend",
		Params: []runtime.ParamSpec{
			{Name: "timeRange", Flag: "time-range", In: "query", GoType: "string", Help: "Required UTC natural dashboard time range. (query, required, one of: DASHBOARD_TIME_RANGE_UNSPECIFIED|DASHBOARD_TIME_RANGE_TODAY|DASHBOARD_TIME_RANGE_THIS_WEEK|DASHBOARD_TIME_RANGE_THIS_MONTH|DASHBOARD_TIME_RANGE_THIS_QUARTER)", Required: true, Default: "DASHBOARD_TIME_RANGE_UNSPECIFIED", Enum: []string{"DASHBOARD_TIME_RANGE_UNSPECIFIED", "DASHBOARD_TIME_RANGE_TODAY", "DASHBOARD_TIME_RANGE_THIS_WEEK", "DASHBOARD_TIME_RANGE_THIS_MONTH", "DASHBOARD_TIME_RANGE_THIS_QUARTER"}},
		},
		Output: runtime.OutputHints{ListPath: "points", DefaultColumns: []string{"time", "totalTokens"}},
	},
	{
		Group:       "BusinessValueService",
		Use:         "get-rated-capacity",
		Short:       "GetRatedCapacity returns the rated capacity value (in millions).",
		OperationID: "BusinessValueService_GetRatedCapacity",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-value/rated-capacity",
		Params: []runtime.ParamSpec{
			{Name: "timeRange", Flag: "time-range", In: "query", GoType: "string", Help: "Required UTC natural dashboard period for capacity calculation. (query, required, one of: DASHBOARD_TIME_RANGE_UNSPECIFIED|DASHBOARD_TIME_RANGE_TODAY|DASHBOARD_TIME_RANGE_THIS_WEEK|DASHBOARD_TIME_RANGE_THIS_MONTH|DASHBOARD_TIME_RANGE_THIS_QUARTER)", Required: true, Default: "DASHBOARD_TIME_RANGE_UNSPECIFIED", Enum: []string{"DASHBOARD_TIME_RANGE_UNSPECIFIED", "DASHBOARD_TIME_RANGE_TODAY", "DASHBOARD_TIME_RANGE_THIS_WEEK", "DASHBOARD_TIME_RANGE_THIS_MONTH", "DASHBOARD_TIME_RANGE_THIS_QUARTER"}},
		},
	},
	{
		Group:       "BusinessValueService",
		Use:         "get-revenue-margin-trend-forecast",
		Short:       "GetRevenueMarginTrendForecast returns the successful response for the request",
		OperationID: "BusinessValueService_GetRevenueMarginTrendForecast",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-value/revenue-margin-trend-forecast",
		Output:      runtime.OutputHints{ListPath: "points", DefaultColumns: []string{"cost", "date", "grossProfit", "revenue"}},
	},
	{
		Group:       "BusinessValueService",
		Use:         "get-risk-suggestions",
		Short:       "GetRiskSuggestions returns the current risk identification and business suggestions.",
		OperationID: "BusinessValueService_GetRiskSuggestions",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-value/risk-suggestions",
		Output:      runtime.OutputHints{ListPath: "suggestions", DefaultColumns: []string{"type", "content"}},
	},
	{
		Group:       "BusinessValueService",
		Use:         "get-summary-metric-thresholds",
		Short:       "GetSummaryMetricThresholds returns business rules for the business-value",
		OperationID: "BusinessValueService_GetSummaryMetricThresholds",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-value/summary-metric-thresholds",
		Output:      runtime.OutputHints{ListPath: "rules", DefaultColumns: []string{"metricId", "operator", "value"}},
	},
	{
		Group:       "BusinessValueService",
		Use:         "get-tenant-consumption-distribution",
		Short:       "GetTenantConsumptionDistribution returns Top10 customer groups and an optional other group.",
		OperationID: "BusinessValueService_GetTenantConsumptionDistribution",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-value/tenant-consumption-distribution",
		Params: []runtime.ParamSpec{
			{Name: "timeRange", Flag: "time-range", In: "query", GoType: "string", Help: "One of \"today\", \"this-week\", \"this-month\", or \"this-quarter\". (query)", Required: false},
		},
		Output: runtime.OutputHints{ListPath: "items", DefaultColumns: []string{"isOther", "revenue", "tenantId", "tenantName", "tokenTotal"}},
	},
	{
		Group:       "BusinessValueService",
		Use:         "get-tenant-growth-trend-top10",
		Short:       "GetTenantGrowthTrendTop10 returns all-model daily token usage for the top ten tenants.",
		OperationID: "BusinessValueService_GetTenantGrowthTrendTop10",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-value/tenant-growth-trend-top10",
		Params: []runtime.ParamSpec{
			{Name: "startTime", Flag: "start-time", In: "query", GoType: "string", Help: "Inclusive start of the requested UTC time window. (query, date-time)", Required: false, Format: "date-time"},
			{Name: "endTime", Flag: "end-time", In: "query", GoType: "string", Help: "Exclusive end of the requested UTC time window. (query, date-time)", Required: false, Format: "date-time"},
		},
	},
	{
		Group:       "BusinessValueService",
		Use:         "get-token-throughput",
		Short:       "GetTokenThroughput returns the current per-minute Token throughput.",
		OperationID: "BusinessValueService_GetTokenThroughput",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-value/token-throughput",
	},
	{
		Group:       "BusinessValueService",
		Use:         "get-token-throughput-history",
		Short:       "GetTokenThroughputHistory returns simulated historical throughput data points.",
		OperationID: "BusinessValueService_GetTokenThroughputHistory",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-value/token-throughput-history",
		Params: []runtime.ParamSpec{
			{Name: "steps", Flag: "steps", In: "query", GoType: "int64", Help: "Maximum number of recent historical data points to return. (query, int32)", Required: false, Format: "int32"},
		},
		Output: runtime.OutputHints{ListPath: "points", DefaultColumns: []string{"timestamp", "tokensPerMinute"}},
	},
	{
		Group:       "BusinessValueService",
		Use:         "get-value-attribution-module-boosts",
		Short:       "GetValueAttributionModuleBoosts returns module boost percentages from config.",
		OperationID: "BusinessValueService_GetValueAttributionModuleBoosts",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/business-value/value-attribution-module-boosts",
		Params: []runtime.ParamSpec{
			{Name: "timeRange", Flag: "time-range", In: "query", GoType: "string", Help: "Required UTC natural dashboard period for cost attribution. (query, required, one of: DASHBOARD_TIME_RANGE_UNSPECIFIED|DASHBOARD_TIME_RANGE_TODAY|DASHBOARD_TIME_RANGE_THIS_WEEK|DASHBOARD_TIME_RANGE_THIS_MONTH|DASHBOARD_TIME_RANGE_THIS_QUARTER)", Required: true, Default: "DASHBOARD_TIME_RANGE_UNSPECIFIED", Enum: []string{"DASHBOARD_TIME_RANGE_UNSPECIFIED", "DASHBOARD_TIME_RANGE_TODAY", "DASHBOARD_TIME_RANGE_THIS_WEEK", "DASHBOARD_TIME_RANGE_THIS_MONTH", "DASHBOARD_TIME_RANGE_THIS_QUARTER"}},
		},
		Output: runtime.OutputHints{ListPath: "moduleItems", DefaultColumns: []string{"costSavings", "moduleName", "sharePercent"}},
	},
	{
		Group:       "ComputePowerCollaborationService",
		Use:         "get-carbon-account",
		Short:       "ComputePowerCollaborationService_GetCarbonAccount",
		OperationID: "ComputePowerCollaborationService_GetCarbonAccount",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/compute-power-collaboration/carbon-account",
		Params: []runtime.ParamSpec{
			{Name: "query.cluster", Flag: "query.cluster", In: "query", GoType: "string", Help: "Optional cluster filter; when empty, all clusters are aggregated. (query)", Required: false},
			{Name: "query.range", Flag: "query.range", In: "query", GoType: "string", Help: "Optional time range token: 24h, 7d or 1m. Defaults to 24h. (query)", Required: false},
			{Name: "query.timezone", Flag: "query.timezone", In: "query", GoType: "string", Help: "Optional IANA timezone (e.g. \"Asia/Shanghai\") for rendering x-axis (query)", Required: false},
		},
		Output: runtime.OutputHints{ListPath: "items", DefaultColumns: []string{"id", "precision", "status", "trend", "trendPrecision", "trendUnit"}},
	},
	{
		Group:       "ComputePowerCollaborationService",
		Use:         "get-carbon-path",
		Short:       "ComputePowerCollaborationService_GetCarbonPath",
		OperationID: "ComputePowerCollaborationService_GetCarbonPath",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/compute-power-collaboration/carbon-path",
		Params: []runtime.ParamSpec{
			{Name: "query.cluster", Flag: "query.cluster", In: "query", GoType: "string", Help: "Optional cluster filter; when empty, all clusters are aggregated. (query)", Required: false},
			{Name: "query.range", Flag: "query.range", In: "query", GoType: "string", Help: "Optional time range token: 24h, 7d or 1m. Defaults to 24h. (query)", Required: false},
			{Name: "query.timezone", Flag: "query.timezone", In: "query", GoType: "string", Help: "Optional IANA timezone (e.g. \"Asia/Shanghai\") for rendering x-axis (query)", Required: false},
		},
		Output: runtime.OutputHints{ListPath: "items", DefaultColumns: []string{"emissionKg", "month", "offsetKg", "targetKg"}},
	},
	{
		Group:       "ComputePowerCollaborationService",
		Use:         "get-clusters",
		Short:       "ComputePowerCollaborationService_GetClusters",
		OperationID: "ComputePowerCollaborationService_GetClusters",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/compute-power-collaboration/clusters",
		Output:      runtime.OutputHints{ListPath: "clusters", DefaultColumns: []string{"name"}},
	},
	{
		Group:       "ComputePowerCollaborationService",
		Use:         "get-dc-regions",
		Short:       "ComputePowerCollaborationService_GetDcRegions",
		OperationID: "ComputePowerCollaborationService_GetDcRegions",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/compute-power-collaboration/dc-regions",
		Params: []runtime.ParamSpec{
			{Name: "query.cluster", Flag: "query.cluster", In: "query", GoType: "string", Help: "Optional cluster filter; when empty, all clusters are aggregated. (query)", Required: false},
			{Name: "query.range", Flag: "query.range", In: "query", GoType: "string", Help: "Optional time range token: 24h, 7d or 1m. Defaults to 24h. (query)", Required: false},
			{Name: "query.timezone", Flag: "query.timezone", In: "query", GoType: "string", Help: "Optional IANA timezone (e.g. \"Asia/Shanghai\") for rendering x-axis (query)", Required: false},
		},
		Output: runtime.OutputHints{ListPath: "items", DefaultColumns: []string{"capacityPct", "cluster", "clusterPhase", "gpuCount", "greenRatioPct", "isOffline"}},
	},
	{
		Group:       "ComputePowerCollaborationService",
		Use:         "get-green-energy",
		Short:       "ComputePowerCollaborationService_GetGreenEnergy",
		OperationID: "ComputePowerCollaborationService_GetGreenEnergy",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/compute-power-collaboration/green-energy",
		Params: []runtime.ParamSpec{
			{Name: "query.cluster", Flag: "query.cluster", In: "query", GoType: "string", Help: "Optional cluster filter; when empty, all clusters are aggregated. (query)", Required: false},
			{Name: "query.range", Flag: "query.range", In: "query", GoType: "string", Help: "Optional time range token: 24h, 7d or 1m. Defaults to 24h. (query)", Required: false},
			{Name: "query.timezone", Flag: "query.timezone", In: "query", GoType: "string", Help: "Optional IANA timezone (e.g. \"Asia/Shanghai\") for rendering x-axis (query)", Required: false},
		},
		Output: runtime.OutputHints{ListPath: "items", DefaultColumns: []string{"id", "precision", "status", "trend", "trendPrecision", "trendUnit"}},
	},
	{
		Group:       "ComputePowerCollaborationService",
		Use:         "get-green-load-scheduling",
		Short:       "ComputePowerCollaborationService_GetGreenLoadScheduling",
		OperationID: "ComputePowerCollaborationService_GetGreenLoadScheduling",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/compute-power-collaboration/green-load-scheduling",
		Params: []runtime.ParamSpec{
			{Name: "query.cluster", Flag: "query.cluster", In: "query", GoType: "string", Help: "Optional cluster filter; when empty, all clusters are aggregated. (query)", Required: false},
			{Name: "query.range", Flag: "query.range", In: "query", GoType: "string", Help: "Optional time range token: 24h, 7d or 1m. Defaults to 24h. (query)", Required: false},
			{Name: "query.timezone", Flag: "query.timezone", In: "query", GoType: "string", Help: "Optional IANA timezone (e.g. \"Asia/Shanghai\") for rendering x-axis (query)", Required: false},
		},
		Output: runtime.OutputHints{ListPath: "items", DefaultColumns: []string{"elastic", "greenRatio", "hour", "rigid"}},
	},
	{
		Group:       "ComputePowerCollaborationService",
		Use:         "get-green-scheduling",
		Short:       "ComputePowerCollaborationService_GetGreenScheduling",
		OperationID: "ComputePowerCollaborationService_GetGreenScheduling",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/compute-power-collaboration/green-scheduling",
		Params: []runtime.ParamSpec{
			{Name: "query.cluster", Flag: "query.cluster", In: "query", GoType: "string", Help: "Optional cluster filter; when empty, all clusters are aggregated. (query)", Required: false},
			{Name: "query.range", Flag: "query.range", In: "query", GoType: "string", Help: "Optional time range token: 24h, 7d or 1m. Defaults to 24h. (query)", Required: false},
			{Name: "query.timezone", Flag: "query.timezone", In: "query", GoType: "string", Help: "Optional IANA timezone (e.g. \"Asia/Shanghai\") for rendering x-axis (query)", Required: false},
		},
		Output: runtime.OutputHints{ListPath: "params", DefaultColumns: []string{"key", "value"}},
	},
	{
		Group:       "ComputePowerCollaborationService",
		Use:         "get-green-suggestions",
		Short:       "ComputePowerCollaborationService_GetGreenSuggestions",
		OperationID: "ComputePowerCollaborationService_GetGreenSuggestions",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/compute-power-collaboration/green-suggestions",
		Params: []runtime.ParamSpec{
			{Name: "query.cluster", Flag: "query.cluster", In: "query", GoType: "string", Help: "Optional cluster filter; when empty, all clusters are aggregated. (query)", Required: false},
			{Name: "query.range", Flag: "query.range", In: "query", GoType: "string", Help: "Optional time range token: 24h, 7d or 1m. Defaults to 24h. (query)", Required: false},
			{Name: "query.timezone", Flag: "query.timezone", In: "query", GoType: "string", Help: "Optional IANA timezone (e.g. \"Asia/Shanghai\") for rendering x-axis (query)", Required: false},
		},
		Output: runtime.OutputHints{ListPath: "items", DefaultColumns: []string{"code", "messageKey", "severity"}},
	},
	{
		Group:       "ComputePowerCollaborationService",
		Use:         "get-green-supply-trend",
		Short:       "ComputePowerCollaborationService_GetGreenSupplyTrend",
		OperationID: "ComputePowerCollaborationService_GetGreenSupplyTrend",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/compute-power-collaboration/green-supply-trend",
		Params: []runtime.ParamSpec{
			{Name: "query.cluster", Flag: "query.cluster", In: "query", GoType: "string", Help: "Optional cluster filter; when empty, all clusters are aggregated. (query)", Required: false},
			{Name: "query.range", Flag: "query.range", In: "query", GoType: "string", Help: "Optional time range token: 24h, 7d or 1m. Defaults to 24h. (query)", Required: false},
			{Name: "query.timezone", Flag: "query.timezone", In: "query", GoType: "string", Help: "Optional IANA timezone (e.g. \"Asia/Shanghai\") for rendering x-axis (query)", Required: false},
		},
		Output: runtime.OutputHints{ListPath: "series", DefaultColumns: []string{"id"}},
	},
	{
		Group:       "ComputePowerCollaborationService",
		Use:         "get-kpis",
		Short:       "ComputePowerCollaborationService_GetKpis",
		OperationID: "ComputePowerCollaborationService_GetKpis",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/compute-power-collaboration/kpis",
		Params: []runtime.ParamSpec{
			{Name: "query.cluster", Flag: "query.cluster", In: "query", GoType: "string", Help: "Optional cluster filter; when empty, all clusters are aggregated. (query)", Required: false},
			{Name: "query.range", Flag: "query.range", In: "query", GoType: "string", Help: "Optional time range token: 24h, 7d or 1m. Defaults to 24h. (query)", Required: false},
			{Name: "query.timezone", Flag: "query.timezone", In: "query", GoType: "string", Help: "Optional IANA timezone (e.g. \"Asia/Shanghai\") for rendering x-axis (query)", Required: false},
		},
		Output: runtime.OutputHints{ListPath: "items", DefaultColumns: []string{"id", "precision", "status", "trend", "trendPrecision", "trendUnit"}},
	},
	{
		Group:       "ComputePowerCollaborationService",
		Use:         "get-power-trend",
		Short:       "ComputePowerCollaborationService_GetPowerTrend",
		OperationID: "ComputePowerCollaborationService_GetPowerTrend",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/compute-power-collaboration/power-trend",
		Params: []runtime.ParamSpec{
			{Name: "query.cluster", Flag: "query.cluster", In: "query", GoType: "string", Help: "Optional cluster filter; when empty, all clusters are aggregated. (query)", Required: false},
			{Name: "query.range", Flag: "query.range", In: "query", GoType: "string", Help: "Optional time range token: 24h, 7d or 1m. Defaults to 24h. (query)", Required: false},
			{Name: "query.timezone", Flag: "query.timezone", In: "query", GoType: "string", Help: "Optional IANA timezone (e.g. \"Asia/Shanghai\") for rendering x-axis (query)", Required: false},
		},
		Output: runtime.OutputHints{ListPath: "series", DefaultColumns: []string{"id"}},
	},
	{
		Group:       "ComputePowerCollaborationService",
		Use:         "get-protection-strategy",
		Short:       "ComputePowerCollaborationService_GetProtectionStrategy",
		OperationID: "ComputePowerCollaborationService_GetProtectionStrategy",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/compute-power-collaboration/protection-strategy",
		Params: []runtime.ParamSpec{
			{Name: "query.cluster", Flag: "query.cluster", In: "query", GoType: "string", Help: "Optional cluster filter; when empty, all clusters are aggregated. (query)", Required: false},
			{Name: "query.range", Flag: "query.range", In: "query", GoType: "string", Help: "Optional time range token: 24h, 7d or 1m. Defaults to 24h. (query)", Required: false},
			{Name: "query.timezone", Flag: "query.timezone", In: "query", GoType: "string", Help: "Optional IANA timezone (e.g. \"Asia/Shanghai\") for rendering x-axis (query)", Required: false},
		},
		Output: runtime.OutputHints{ListPath: "greenRules", DefaultColumns: []string{"band", "impactCode", "strategyCode", "targetCode"}},
	},
	{
		Group:       "ComputePowerCollaborationService",
		Use:         "get-suggestions",
		Short:       "ComputePowerCollaborationService_GetSuggestions",
		OperationID: "ComputePowerCollaborationService_GetSuggestions",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/compute-power-collaboration/suggestions",
		Params: []runtime.ParamSpec{
			{Name: "query.cluster", Flag: "query.cluster", In: "query", GoType: "string", Help: "Optional cluster filter; when empty, all clusters are aggregated. (query)", Required: false},
			{Name: "query.range", Flag: "query.range", In: "query", GoType: "string", Help: "Optional time range token: 24h, 7d or 1m. Defaults to 24h. (query)", Required: false},
			{Name: "query.timezone", Flag: "query.timezone", In: "query", GoType: "string", Help: "Optional IANA timezone (e.g. \"Asia/Shanghai\") for rendering x-axis (query)", Required: false},
		},
		Output: runtime.OutputHints{ListPath: "items", DefaultColumns: []string{"code", "messageKey", "severity"}},
	},
	{
		Group:       "ComputePowerCollaborationService",
		Use:         "get-synergy-value",
		Short:       "ComputePowerCollaborationService_GetSynergyValue",
		OperationID: "ComputePowerCollaborationService_GetSynergyValue",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/compute-power-collaboration/synergy-value",
		Params: []runtime.ParamSpec{
			{Name: "query.cluster", Flag: "query.cluster", In: "query", GoType: "string", Help: "Optional cluster filter; when empty, all clusters are aggregated. (query)", Required: false},
			{Name: "query.range", Flag: "query.range", In: "query", GoType: "string", Help: "Optional time range token: 24h, 7d or 1m. Defaults to 24h. (query)", Required: false},
			{Name: "query.timezone", Flag: "query.timezone", In: "query", GoType: "string", Help: "Optional IANA timezone (e.g. \"Asia/Shanghai\") for rendering x-axis (query)", Required: false},
		},
		Output: runtime.OutputHints{ListPath: "items", DefaultColumns: []string{"id", "precision", "status", "trend", "trendPrecision", "trendUnit"}},
	},
	{
		Group:       "ExpenseService",
		Use:         "get-balance-voucher-overview",
		Short:       "GetBalanceVoucherOverview returns balance/voucher stock and flow points.",
		OperationID: "ExpenseService_GetBalanceVoucherOverview",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/expense/balance-voucher-overview",
		Params: []runtime.ParamSpec{
			{Name: "range", Flag: "range", In: "query", GoType: "string", Help: "range (query, required, one of: BALANCE_VOUCHER_OVERVIEW_RANGE_UNSPECIFIED|BALANCE_VOUCHER_OVERVIEW_RANGE_LAST_30_DAYS|BALANCE_VOUCHER_OVERVIEW_RANGE_LAST_12_MONTHS)", Required: true, Default: "BALANCE_VOUCHER_OVERVIEW_RANGE_UNSPECIFIED", Enum: []string{"BALANCE_VOUCHER_OVERVIEW_RANGE_UNSPECIFIED", "BALANCE_VOUCHER_OVERVIEW_RANGE_LAST_30_DAYS", "BALANCE_VOUCHER_OVERVIEW_RANGE_LAST_12_MONTHS"}},
			{Name: "timeZone", Flag: "time-zone", In: "query", GoType: "string", Help: "timeZone (query, required)", Required: true},
		},
		Output: runtime.OutputHints{ListPath: "points", DefaultColumns: []string{"balanceAvailable", "balanceTotalMicros", "bucketStart", "rechargeAmountMicros", "rechargeAvailable", "voucherEffectiveAmountMicros"}},
	},
	{
		Group:       "ExpenseService",
		Use:         "get-platform-revenue-analytics",
		Short:       "GetPlatformRevenueAnalytics returns ordered balance-paid and voucher-consumption",
		OperationID: "ExpenseService_GetPlatformRevenueAnalytics",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/expense/platform-revenue-analytics",
		Params: []runtime.ParamSpec{
			{Name: "range", Flag: "range", In: "query", GoType: "string", Help: "range (query, required, one of: PLATFORM_REVENUE_RANGE_UNSPECIFIED|PLATFORM_REVENUE_RANGE_LAST_30_DAYS|PLATFORM_REVENUE_RANGE_CURRENT_YEAR)", Required: true, Default: "PLATFORM_REVENUE_RANGE_UNSPECIFIED", Enum: []string{"PLATFORM_REVENUE_RANGE_UNSPECIFIED", "PLATFORM_REVENUE_RANGE_LAST_30_DAYS", "PLATFORM_REVENUE_RANGE_CURRENT_YEAR"}},
			{Name: "timeZone", Flag: "time-zone", In: "query", GoType: "string", Help: "IANA time-zone name used to derive local day and month boundaries. (query, required)", Required: true},
		},
		Output: runtime.OutputHints{ListPath: "points", DefaultColumns: []string{"bucketStart", "rechargeAmountMicros", "voucherConsumptionAmountMicros"}},
	},
	{
		Group:       "FinopsPanelService",
		Use:         "get-allocation-summary",
		Short:       "FinopsPanelService_GetAllocationSummary",
		OperationID: "FinopsPanelService_GetAllocationSummary",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/finops/allocation-summary",
		Params: []runtime.ParamSpec{
			{Name: "financialPeriod", Flag: "financial-period", In: "query", GoType: "string", Help: "financialPeriod (query, one of: FINANCIAL_PERIOD_THIS_MONTH|FINANCIAL_PERIOD_LAST_MONTH|FINANCIAL_PERIOD_THIS_QUARTER|FINANCIAL_PERIOD_THIS_YEAR)", Required: false, Default: "FINANCIAL_PERIOD_THIS_MONTH", Enum: []string{"FINANCIAL_PERIOD_THIS_MONTH", "FINANCIAL_PERIOD_LAST_MONTH", "FINANCIAL_PERIOD_THIS_QUARTER", "FINANCIAL_PERIOD_THIS_YEAR"}},
			{Name: "accountingScope", Flag: "accounting-scope", In: "query", GoType: "string", Help: "accountingScope (query, one of: ACCOUNTING_SCOPE_MERGED|ACCOUNTING_SCOPE_EXTERNAL_REVENUE|ACCOUNTING_SCOPE_INTERNAL_ALLOCATION)", Required: false, Default: "ACCOUNTING_SCOPE_MERGED", Enum: []string{"ACCOUNTING_SCOPE_MERGED", "ACCOUNTING_SCOPE_EXTERNAL_REVENUE", "ACCOUNTING_SCOPE_INTERNAL_ALLOCATION"}},
		},
		Output: runtime.OutputHints{ListPath: "rows", DefaultColumns: []string{"type", "allocCost", "object", "revenue", "roi", "tokenUsage"}},
	},
	{
		Group:       "FinopsPanelService",
		Use:         "get-asset-return-matrix",
		Short:       "FinopsPanelService_GetAssetReturnMatrix",
		OperationID: "FinopsPanelService_GetAssetReturnMatrix",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/finops/asset-return-matrix",
		Params: []runtime.ParamSpec{
			{Name: "financialPeriod", Flag: "financial-period", In: "query", GoType: "string", Help: "financialPeriod (query, one of: FINANCIAL_PERIOD_THIS_MONTH|FINANCIAL_PERIOD_LAST_MONTH|FINANCIAL_PERIOD_THIS_QUARTER|FINANCIAL_PERIOD_THIS_YEAR)", Required: false, Default: "FINANCIAL_PERIOD_THIS_MONTH", Enum: []string{"FINANCIAL_PERIOD_THIS_MONTH", "FINANCIAL_PERIOD_LAST_MONTH", "FINANCIAL_PERIOD_THIS_QUARTER", "FINANCIAL_PERIOD_THIS_YEAR"}},
			{Name: "accountingScope", Flag: "accounting-scope", In: "query", GoType: "string", Help: "accountingScope (query, one of: ACCOUNTING_SCOPE_MERGED|ACCOUNTING_SCOPE_EXTERNAL_REVENUE|ACCOUNTING_SCOPE_INTERNAL_ALLOCATION)", Required: false, Default: "ACCOUNTING_SCOPE_MERGED", Enum: []string{"ACCOUNTING_SCOPE_MERGED", "ACCOUNTING_SCOPE_EXTERNAL_REVENUE", "ACCOUNTING_SCOPE_INTERNAL_ALLOCATION"}},
		},
		Output: runtime.OutputHints{ListPath: "points", DefaultColumns: []string{"bookValue", "machine", "profitValue", "status", "utilization"}},
	},
	{
		Group:       "FinopsPanelService",
		Use:         "get-budget-forecast",
		Short:       "FinopsPanelService_GetBudgetForecast",
		OperationID: "FinopsPanelService_GetBudgetForecast",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/finops/budget-forecast",
		Params: []runtime.ParamSpec{
			{Name: "financialPeriod", Flag: "financial-period", In: "query", GoType: "string", Help: "financialPeriod (query, one of: FINANCIAL_PERIOD_THIS_MONTH|FINANCIAL_PERIOD_LAST_MONTH|FINANCIAL_PERIOD_THIS_QUARTER|FINANCIAL_PERIOD_THIS_YEAR)", Required: false, Default: "FINANCIAL_PERIOD_THIS_MONTH", Enum: []string{"FINANCIAL_PERIOD_THIS_MONTH", "FINANCIAL_PERIOD_LAST_MONTH", "FINANCIAL_PERIOD_THIS_QUARTER", "FINANCIAL_PERIOD_THIS_YEAR"}},
			{Name: "accountingScope", Flag: "accounting-scope", In: "query", GoType: "string", Help: "accountingScope (query, one of: ACCOUNTING_SCOPE_MERGED|ACCOUNTING_SCOPE_EXTERNAL_REVENUE|ACCOUNTING_SCOPE_INTERNAL_ALLOCATION)", Required: false, Default: "ACCOUNTING_SCOPE_MERGED", Enum: []string{"ACCOUNTING_SCOPE_MERGED", "ACCOUNTING_SCOPE_EXTERNAL_REVENUE", "ACCOUNTING_SCOPE_INTERNAL_ALLOCATION"}},
		},
		Output: runtime.OutputHints{ListPath: "categories"},
	},
	{
		Group:       "FinopsPanelService",
		Use:         "get-budget-remaining",
		Short:       "FinopsPanelService_GetBudgetRemaining",
		OperationID: "FinopsPanelService_GetBudgetRemaining",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/finops/budget-remaining",
		Params: []runtime.ParamSpec{
			{Name: "financialPeriod", Flag: "financial-period", In: "query", GoType: "string", Help: "financialPeriod (query, one of: FINANCIAL_PERIOD_THIS_MONTH|FINANCIAL_PERIOD_LAST_MONTH|FINANCIAL_PERIOD_THIS_QUARTER|FINANCIAL_PERIOD_THIS_YEAR)", Required: false, Default: "FINANCIAL_PERIOD_THIS_MONTH", Enum: []string{"FINANCIAL_PERIOD_THIS_MONTH", "FINANCIAL_PERIOD_LAST_MONTH", "FINANCIAL_PERIOD_THIS_QUARTER", "FINANCIAL_PERIOD_THIS_YEAR"}},
			{Name: "accountingScope", Flag: "accounting-scope", In: "query", GoType: "string", Help: "accountingScope (query, one of: ACCOUNTING_SCOPE_MERGED|ACCOUNTING_SCOPE_EXTERNAL_REVENUE|ACCOUNTING_SCOPE_INTERNAL_ALLOCATION)", Required: false, Default: "ACCOUNTING_SCOPE_MERGED", Enum: []string{"ACCOUNTING_SCOPE_MERGED", "ACCOUNTING_SCOPE_EXTERNAL_REVENUE", "ACCOUNTING_SCOPE_INTERNAL_ALLOCATION"}},
		},
	},
	{
		Group:       "FinopsPanelService",
		Use:         "get-cost-structure-breakdown",
		Short:       "FinopsPanelService_GetCostStructureBreakdown",
		OperationID: "FinopsPanelService_GetCostStructureBreakdown",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/finops/cost-structure-breakdown",
		Params: []runtime.ParamSpec{
			{Name: "financialPeriod", Flag: "financial-period", In: "query", GoType: "string", Help: "financialPeriod (query, one of: FINANCIAL_PERIOD_THIS_MONTH|FINANCIAL_PERIOD_LAST_MONTH|FINANCIAL_PERIOD_THIS_QUARTER|FINANCIAL_PERIOD_THIS_YEAR)", Required: false, Default: "FINANCIAL_PERIOD_THIS_MONTH", Enum: []string{"FINANCIAL_PERIOD_THIS_MONTH", "FINANCIAL_PERIOD_LAST_MONTH", "FINANCIAL_PERIOD_THIS_QUARTER", "FINANCIAL_PERIOD_THIS_YEAR"}},
			{Name: "accountingScope", Flag: "accounting-scope", In: "query", GoType: "string", Help: "accountingScope (query, one of: ACCOUNTING_SCOPE_MERGED|ACCOUNTING_SCOPE_EXTERNAL_REVENUE|ACCOUNTING_SCOPE_INTERNAL_ALLOCATION)", Required: false, Default: "ACCOUNTING_SCOPE_MERGED", Enum: []string{"ACCOUNTING_SCOPE_MERGED", "ACCOUNTING_SCOPE_EXTERNAL_REVENUE", "ACCOUNTING_SCOPE_INTERNAL_ALLOCATION"}},
		},
		Output: runtime.OutputHints{ListPath: "items", DefaultColumns: []string{"name", "displayValue", "rank", "value"}},
	},
	{
		Group:       "FinopsPanelService",
		Use:         "get-expansion-impact",
		Short:       "FinopsPanelService_GetExpansionImpact",
		OperationID: "FinopsPanelService_GetExpansionImpact",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/finops/expansion-impact",
		Params: []runtime.ParamSpec{
			{Name: "financialPeriod", Flag: "financial-period", In: "query", GoType: "string", Help: "financialPeriod (query, one of: FINANCIAL_PERIOD_THIS_MONTH|FINANCIAL_PERIOD_LAST_MONTH|FINANCIAL_PERIOD_THIS_QUARTER|FINANCIAL_PERIOD_THIS_YEAR)", Required: false, Default: "FINANCIAL_PERIOD_THIS_MONTH", Enum: []string{"FINANCIAL_PERIOD_THIS_MONTH", "FINANCIAL_PERIOD_LAST_MONTH", "FINANCIAL_PERIOD_THIS_QUARTER", "FINANCIAL_PERIOD_THIS_YEAR"}},
			{Name: "accountingScope", Flag: "accounting-scope", In: "query", GoType: "string", Help: "accountingScope (query, one of: ACCOUNTING_SCOPE_MERGED|ACCOUNTING_SCOPE_EXTERNAL_REVENUE|ACCOUNTING_SCOPE_INTERNAL_ALLOCATION)", Required: false, Default: "ACCOUNTING_SCOPE_MERGED", Enum: []string{"ACCOUNTING_SCOPE_MERGED", "ACCOUNTING_SCOPE_EXTERNAL_REVENUE", "ACCOUNTING_SCOPE_INTERNAL_ALLOCATION"}},
		},
		Output: runtime.OutputHints{ListPath: "rows", DefaultColumns: []string{"action", "addedCost", "existingQuantity", "expectedRevenue", "hasMonthlyBudgetImpact", "hasProfitImpact"}},
	},
	{
		Group:       "FinopsPanelService",
		Use:         "get-finops-suggestions",
		Short:       "FinopsPanelService_GetFinopsSuggestions",
		OperationID: "FinopsPanelService_GetFinopsSuggestions",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/finops/suggestions",
		Params: []runtime.ParamSpec{
			{Name: "financialPeriod", Flag: "financial-period", In: "query", GoType: "string", Help: "financialPeriod (query, one of: FINANCIAL_PERIOD_THIS_MONTH|FINANCIAL_PERIOD_LAST_MONTH|FINANCIAL_PERIOD_THIS_QUARTER|FINANCIAL_PERIOD_THIS_YEAR)", Required: false, Default: "FINANCIAL_PERIOD_THIS_MONTH", Enum: []string{"FINANCIAL_PERIOD_THIS_MONTH", "FINANCIAL_PERIOD_LAST_MONTH", "FINANCIAL_PERIOD_THIS_QUARTER", "FINANCIAL_PERIOD_THIS_YEAR"}},
			{Name: "accountingScope", Flag: "accounting-scope", In: "query", GoType: "string", Help: "accountingScope (query, one of: ACCOUNTING_SCOPE_MERGED|ACCOUNTING_SCOPE_EXTERNAL_REVENUE|ACCOUNTING_SCOPE_INTERNAL_ALLOCATION)", Required: false, Default: "ACCOUNTING_SCOPE_MERGED", Enum: []string{"ACCOUNTING_SCOPE_MERGED", "ACCOUNTING_SCOPE_EXTERNAL_REVENUE", "ACCOUNTING_SCOPE_INTERNAL_ALLOCATION"}},
		},
		Output: runtime.OutputHints{ListPath: "items", DefaultColumns: []string{"type", "content"}},
	},
	{
		Group:       "FinopsPanelService",
		Use:         "get-machine-asset-ranking",
		Short:       "FinopsPanelService_GetMachineAssetRanking",
		OperationID: "FinopsPanelService_GetMachineAssetRanking",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/finops/machine-asset-ranking",
		Params: []runtime.ParamSpec{
			{Name: "financialPeriod", Flag: "financial-period", In: "query", GoType: "string", Help: "financialPeriod (query, one of: FINANCIAL_PERIOD_THIS_MONTH|FINANCIAL_PERIOD_LAST_MONTH|FINANCIAL_PERIOD_THIS_QUARTER|FINANCIAL_PERIOD_THIS_YEAR)", Required: false, Default: "FINANCIAL_PERIOD_THIS_MONTH", Enum: []string{"FINANCIAL_PERIOD_THIS_MONTH", "FINANCIAL_PERIOD_LAST_MONTH", "FINANCIAL_PERIOD_THIS_QUARTER", "FINANCIAL_PERIOD_THIS_YEAR"}},
			{Name: "accountingScope", Flag: "accounting-scope", In: "query", GoType: "string", Help: "accountingScope (query, one of: ACCOUNTING_SCOPE_MERGED|ACCOUNTING_SCOPE_EXTERNAL_REVENUE|ACCOUNTING_SCOPE_INTERNAL_ALLOCATION)", Required: false, Default: "ACCOUNTING_SCOPE_MERGED", Enum: []string{"ACCOUNTING_SCOPE_MERGED", "ACCOUNTING_SCOPE_EXTERNAL_REVENUE", "ACCOUNTING_SCOPE_INTERNAL_ALLOCATION"}},
		},
		Output: runtime.OutputHints{ListPath: "rows", DefaultColumns: []string{"bookValue", "config", "depreciation", "machine", "monthToken", "payback"}},
	},
	{
		Group:       "FinopsPanelService",
		Use:         "get-monthly-profit",
		Short:       "FinopsPanelService_GetMonthlyProfit",
		OperationID: "FinopsPanelService_GetMonthlyProfit",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/finops/monthly-profit",
		Params: []runtime.ParamSpec{
			{Name: "financialPeriod", Flag: "financial-period", In: "query", GoType: "string", Help: "financialPeriod (query, one of: FINANCIAL_PERIOD_THIS_MONTH|FINANCIAL_PERIOD_LAST_MONTH|FINANCIAL_PERIOD_THIS_QUARTER|FINANCIAL_PERIOD_THIS_YEAR)", Required: false, Default: "FINANCIAL_PERIOD_THIS_MONTH", Enum: []string{"FINANCIAL_PERIOD_THIS_MONTH", "FINANCIAL_PERIOD_LAST_MONTH", "FINANCIAL_PERIOD_THIS_QUARTER", "FINANCIAL_PERIOD_THIS_YEAR"}},
			{Name: "accountingScope", Flag: "accounting-scope", In: "query", GoType: "string", Help: "accountingScope (query, one of: ACCOUNTING_SCOPE_MERGED|ACCOUNTING_SCOPE_EXTERNAL_REVENUE|ACCOUNTING_SCOPE_INTERNAL_ALLOCATION)", Required: false, Default: "ACCOUNTING_SCOPE_MERGED", Enum: []string{"ACCOUNTING_SCOPE_MERGED", "ACCOUNTING_SCOPE_EXTERNAL_REVENUE", "ACCOUNTING_SCOPE_INTERNAL_ALLOCATION"}},
		},
	},
	{
		Group:       "FinopsPanelService",
		Use:         "get-overview",
		Short:       "FinopsPanelService_GetOverview",
		OperationID: "FinopsPanelService_GetOverview",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/finops/overview",
		Params: []runtime.ParamSpec{
			{Name: "financialPeriod", Flag: "financial-period", In: "query", GoType: "string", Help: "financialPeriod (query, one of: FINANCIAL_PERIOD_THIS_MONTH|FINANCIAL_PERIOD_LAST_MONTH|FINANCIAL_PERIOD_THIS_QUARTER|FINANCIAL_PERIOD_THIS_YEAR)", Required: false, Default: "FINANCIAL_PERIOD_THIS_MONTH", Enum: []string{"FINANCIAL_PERIOD_THIS_MONTH", "FINANCIAL_PERIOD_LAST_MONTH", "FINANCIAL_PERIOD_THIS_QUARTER", "FINANCIAL_PERIOD_THIS_YEAR"}},
			{Name: "accountingScope", Flag: "accounting-scope", In: "query", GoType: "string", Help: "accountingScope (query, one of: ACCOUNTING_SCOPE_MERGED|ACCOUNTING_SCOPE_EXTERNAL_REVENUE|ACCOUNTING_SCOPE_INTERNAL_ALLOCATION)", Required: false, Default: "ACCOUNTING_SCOPE_MERGED", Enum: []string{"ACCOUNTING_SCOPE_MERGED", "ACCOUNTING_SCOPE_EXTERNAL_REVENUE", "ACCOUNTING_SCOPE_INTERNAL_ALLOCATION"}},
		},
	},
	{
		Group:       "FinopsPanelService",
		Use:         "get-revenue-cost-trend",
		Short:       "FinopsPanelService_GetRevenueCostTrend",
		OperationID: "FinopsPanelService_GetRevenueCostTrend",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/finops/revenue-cost-trend",
		Params: []runtime.ParamSpec{
			{Name: "financialPeriod", Flag: "financial-period", In: "query", GoType: "string", Help: "financialPeriod (query, one of: FINANCIAL_PERIOD_THIS_MONTH|FINANCIAL_PERIOD_LAST_MONTH|FINANCIAL_PERIOD_THIS_QUARTER|FINANCIAL_PERIOD_THIS_YEAR)", Required: false, Default: "FINANCIAL_PERIOD_THIS_MONTH", Enum: []string{"FINANCIAL_PERIOD_THIS_MONTH", "FINANCIAL_PERIOD_LAST_MONTH", "FINANCIAL_PERIOD_THIS_QUARTER", "FINANCIAL_PERIOD_THIS_YEAR"}},
			{Name: "accountingScope", Flag: "accounting-scope", In: "query", GoType: "string", Help: "accountingScope (query, one of: ACCOUNTING_SCOPE_MERGED|ACCOUNTING_SCOPE_EXTERNAL_REVENUE|ACCOUNTING_SCOPE_INTERNAL_ALLOCATION)", Required: false, Default: "ACCOUNTING_SCOPE_MERGED", Enum: []string{"ACCOUNTING_SCOPE_MERGED", "ACCOUNTING_SCOPE_EXTERNAL_REVENUE", "ACCOUNTING_SCOPE_INTERNAL_ALLOCATION"}},
		},
		Output: runtime.OutputHints{ListPath: "categories"},
	},
	{
		Group:       "FinopsPanelService",
		Use:         "get-unit-token-cost",
		Short:       "FinopsPanelService_GetUnitTokenCost",
		OperationID: "FinopsPanelService_GetUnitTokenCost",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/finops/unit-token-cost",
		Params: []runtime.ParamSpec{
			{Name: "financialPeriod", Flag: "financial-period", In: "query", GoType: "string", Help: "financialPeriod (query, one of: FINANCIAL_PERIOD_THIS_MONTH|FINANCIAL_PERIOD_LAST_MONTH|FINANCIAL_PERIOD_THIS_QUARTER|FINANCIAL_PERIOD_THIS_YEAR)", Required: false, Default: "FINANCIAL_PERIOD_THIS_MONTH", Enum: []string{"FINANCIAL_PERIOD_THIS_MONTH", "FINANCIAL_PERIOD_LAST_MONTH", "FINANCIAL_PERIOD_THIS_QUARTER", "FINANCIAL_PERIOD_THIS_YEAR"}},
			{Name: "accountingScope", Flag: "accounting-scope", In: "query", GoType: "string", Help: "accountingScope (query, one of: ACCOUNTING_SCOPE_MERGED|ACCOUNTING_SCOPE_EXTERNAL_REVENUE|ACCOUNTING_SCOPE_INTERNAL_ALLOCATION)", Required: false, Default: "ACCOUNTING_SCOPE_MERGED", Enum: []string{"ACCOUNTING_SCOPE_MERGED", "ACCOUNTING_SCOPE_EXTERNAL_REVENUE", "ACCOUNTING_SCOPE_INTERNAL_ALLOCATION"}},
		},
	},
	{
		Group:       "ModelMetricsService",
		Use:         "get-active-model-metrics",
		Short:       "GetActiveModelMetrics returns active-model counts for a UTC natural time range.",
		OperationID: "ModelMetricsService_GetActiveModelMetrics",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/model-metrics/active-model-metrics",
		Params: []runtime.ParamSpec{
			{Name: "timeRange", Flag: "time-range", In: "query", GoType: "string", Help: "Required UTC natural dashboard period. (query, required, one of: DASHBOARD_TIME_RANGE_UNSPECIFIED|DASHBOARD_TIME_RANGE_TODAY|DASHBOARD_TIME_RANGE_THIS_WEEK|DASHBOARD_TIME_RANGE_THIS_MONTH|DASHBOARD_TIME_RANGE_THIS_QUARTER)", Required: true, Default: "DASHBOARD_TIME_RANGE_UNSPECIFIED", Enum: []string{"DASHBOARD_TIME_RANGE_UNSPECIFIED", "DASHBOARD_TIME_RANGE_TODAY", "DASHBOARD_TIME_RANGE_THIS_WEEK", "DASHBOARD_TIME_RANGE_THIS_MONTH", "DASHBOARD_TIME_RANGE_THIS_QUARTER"}},
		},
	},
	{
		Group:       "PlatformConfigService",
		Use:         "get-runtime-mode",
		Short:       "GetRuntimeMode returns the current deployment runtime mode (csp or ws).",
		OperationID: "PlatformConfigService_GetRuntimeMode",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/platform-config/runtime-mode",
	},
	{
		Group:       "ProductionOperationsService",
		Use:         "get-realtime-headlines",
		Short:       "ProductionOperationsService_GetRealtimeHeadlines",
		OperationID: "ProductionOperationsService_GetRealtimeHeadlines",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/production-ops/realtime/headlines",
		Params: []runtime.ParamSpec{
			{Name: "cluster", Flag: "cluster", In: "query", GoType: "string", Help: "cluster (query)", Required: false},
		},
		Output: runtime.OutputHints{ListPath: "groupStates", DefaultColumns: []string{"key", "message", "status"}},
	},
	{
		Group:       "ProductionOperationsService",
		Use:         "get-realtime-tab",
		Short:       "ProductionOperationsService_GetRealtimeTab",
		OperationID: "ProductionOperationsService_GetRealtimeTab",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/production-ops/realtime",
		Params: []runtime.ParamSpec{
			{Name: "timeWindow", Flag: "time-window", In: "query", GoType: "string", Help: "timeWindow (query)", Required: false},
			{Name: "cluster", Flag: "cluster", In: "query", GoType: "string", Help: "cluster (query)", Required: false},
		},
		Output: runtime.OutputHints{ListPath: "groupStates", DefaultColumns: []string{"key", "message", "status"}},
	},
	{
		Group:       "ResourceCostService",
		Use:         "get-cost-kpis",
		Short:       "GetCostKpis returns GPU KPI summary metrics.",
		OperationID: "ResourceCostService_GetCostKpis",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/resource-cost/kpis",
		Params: []runtime.ParamSpec{
			{Name: "cluster", Flag: "cluster", In: "query", GoType: "string", Help: "Optional cluster name filter. Empty means all clusters. (query)", Required: false},
			{Name: "timeRange", Flag: "time-range", In: "query", GoType: "string", Help: "Time range for token/revenue queries. Default is THIS_MONTH. (query, one of: THIS_MONTH|THIS_WEEK|LAST_MONTH|LAST_7_DAYS|LAST_30_DAYS)", Required: false, Default: "THIS_MONTH", Enum: []string{"THIS_MONTH", "THIS_WEEK", "LAST_MONTH", "LAST_7_DAYS", "LAST_30_DAYS"}},
		},
	},
	{
		Group:       "ResourceCostService",
		Use:         "get-cost-optimization",
		Short:       "GetCostOptimization returns cost optimization measures.",
		OperationID: "ResourceCostService_GetCostOptimization",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/resource-cost/optimization",
		Output:      runtime.OutputHints{ListPath: "items", DefaultColumns: []string{"measure", "value"}},
	},
	{
		Group:       "ResourceCostService",
		Use:         "get-cost-suggestions",
		Short:       "GetCostSuggestions returns dynamic optimization suggestions.",
		OperationID: "ResourceCostService_GetCostSuggestions",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/resource-cost/suggestions",
		Params: []runtime.ParamSpec{
			{Name: "cluster", Flag: "cluster", In: "query", GoType: "string", Help: "Optional cluster name filter. Empty means all clusters. (query)", Required: false},
			{Name: "timeRange", Flag: "time-range", In: "query", GoType: "string", Help: "Time range for token/revenue queries. Default is THIS_MONTH. (query, one of: THIS_MONTH|THIS_WEEK|LAST_MONTH|LAST_7_DAYS|LAST_30_DAYS)", Required: false, Default: "THIS_MONTH", Enum: []string{"THIS_MONTH", "THIS_WEEK", "LAST_MONTH", "LAST_7_DAYS", "LAST_30_DAYS"}},
		},
		Output: runtime.OutputHints{ListPath: "items", DefaultColumns: []string{"type", "content"}},
	},
	{
		Group:       "ResourceCostService",
		Use:         "get-gpu-efficiency",
		Short:       "GetGpuEfficiency returns GPU model efficiency ranking.",
		OperationID: "ResourceCostService_GetGpuEfficiency",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/resource-cost/gpu-efficiency",
		Params: []runtime.ParamSpec{
			{Name: "cluster", Flag: "cluster", In: "query", GoType: "string", Help: "Optional cluster name filter. Empty means all clusters. (query)", Required: false},
			{Name: "timeRange", Flag: "time-range", In: "query", GoType: "string", Help: "Time range for token/revenue queries. Default is THIS_MONTH. (query, one of: THIS_MONTH|THIS_WEEK|LAST_MONTH|LAST_7_DAYS|LAST_30_DAYS)", Required: false, Default: "THIS_MONTH", Enum: []string{"THIS_MONTH", "THIS_WEEK", "LAST_MONTH", "LAST_7_DAYS", "LAST_30_DAYS"}},
		},
		Output: runtime.OutputHints{ListPath: "rows", DefaultColumns: []string{"count", "dailyCost", "dailyOutput", "dailyProfit", "gpuModel", "memUtilization"}},
	},
	{
		Group:       "ResourceCostService",
		Use:         "get-model-cost-rank",
		Short:       "GetModelCostRank returns model cost ranking.",
		OperationID: "ResourceCostService_GetModelCostRank",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/resource-cost/model-cost-rank",
		Params: []runtime.ParamSpec{
			{Name: "cluster", Flag: "cluster", In: "query", GoType: "string", Help: "Optional cluster name filter. Empty means all clusters. (query)", Required: false},
			{Name: "timeRange", Flag: "time-range", In: "query", GoType: "string", Help: "Time range for token/revenue queries. Default is THIS_MONTH. (query, one of: THIS_MONTH|THIS_WEEK|LAST_MONTH|LAST_7_DAYS|LAST_30_DAYS)", Required: false, Default: "THIS_MONTH", Enum: []string{"THIS_MONTH", "THIS_WEEK", "LAST_MONTH", "LAST_7_DAYS", "LAST_30_DAYS"}},
		},
		Output: runtime.OutputHints{ListPath: "items", DefaultColumns: []string{"name", "costPerMToken", "deployType", "rank"}},
	},
	{
		Group:       "ResourceCostService",
		Use:         "list-cost-clusters",
		Short:       "ListCostClusters returns all cluster names that have GPU nodes.",
		OperationID: "ResourceCostService_ListCostClusters",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/resource-cost/clusters",
		Output:      runtime.OutputHints{ListPath: "clusters"},
	},
	{
		Group:       "SecurityProtectionService",
		Use:         "get-agent-protection",
		Short:       "SecurityProtectionService_GetAgentProtection",
		OperationID: "SecurityProtectionService_GetAgentProtection",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/security-protection/agent-protection",
		Params: []runtime.ParamSpec{
			{Name: "hours", Flag: "hours", In: "query", GoType: "int64", Help: "time window in hours (1-720, default 24) (query, int32)", Required: false, Format: "int32"},
			{Name: "cluster", Flag: "cluster", In: "query", GoType: "string", Help: "filter by cluster name (query)", Required: false},
			{Name: "level", Flag: "level", In: "query", GoType: "string", Help: "filter by risk level: high/medium/low (query)", Required: false},
		},
		Output: runtime.OutputHints{ListPath: "rows", DefaultColumns: []string{"dimension", "intercepted", "status", "todayCount"}},
	},
	{
		Group:       "SecurityProtectionService",
		Use:         "get-clusters",
		Short:       "SecurityProtectionService_GetClusters",
		OperationID: "SecurityProtectionService_GetClusters",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/security-protection/clusters",
		Output:      runtime.OutputHints{ListPath: "clusters", DefaultColumns: []string{"name", "label"}},
	},
	{
		Group:       "SecurityProtectionService",
		Use:         "get-intercept-trend",
		Short:       "SecurityProtectionService_GetInterceptTrend",
		OperationID: "SecurityProtectionService_GetInterceptTrend",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/security-protection/intercept-trend",
		Params: []runtime.ParamSpec{
			{Name: "hours", Flag: "hours", In: "query", GoType: "int64", Help: "time window in hours (1-720, default 24) (query, int32)", Required: false, Format: "int32"},
			{Name: "cluster", Flag: "cluster", In: "query", GoType: "string", Help: "filter by cluster name (query)", Required: false},
			{Name: "level", Flag: "level", In: "query", GoType: "string", Help: "filter by risk level: high/medium/low (query)", Required: false},
		},
		Output: runtime.OutputHints{ListPath: "data"},
	},
	{
		Group:       "SecurityProtectionService",
		Use:         "get-kpis",
		Short:       "SecurityProtectionService_GetKpis",
		OperationID: "SecurityProtectionService_GetKpis",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/security-protection/kpis",
		Params: []runtime.ParamSpec{
			{Name: "hours", Flag: "hours", In: "query", GoType: "int64", Help: "time window in hours (1-720, default 24) (query, int32)", Required: false, Format: "int32"},
			{Name: "cluster", Flag: "cluster", In: "query", GoType: "string", Help: "filter by cluster name (query)", Required: false},
			{Name: "level", Flag: "level", In: "query", GoType: "string", Help: "filter by risk level: high/medium/low (query)", Required: false},
		},
		Output: runtime.OutputHints{ListPath: "items", DefaultColumns: []string{"id", "color", "label", "value"}},
	},
	{
		Group:       "SecurityProtectionService",
		Use:         "get-output-protection",
		Short:       "SecurityProtectionService_GetOutputProtection",
		OperationID: "SecurityProtectionService_GetOutputProtection",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/security-protection/output-protection",
		Params: []runtime.ParamSpec{
			{Name: "hours", Flag: "hours", In: "query", GoType: "int64", Help: "time window in hours (1-720, default 24) (query, int32)", Required: false, Format: "int32"},
			{Name: "cluster", Flag: "cluster", In: "query", GoType: "string", Help: "filter by cluster name (query)", Required: false},
			{Name: "level", Flag: "level", In: "query", GoType: "string", Help: "filter by risk level: high/medium/low (query)", Required: false},
		},
		Output: runtime.OutputHints{ListPath: "rows", DefaultColumns: []string{"dimension", "intercepted", "status", "todayCount"}},
	},
	{
		Group:       "SecurityProtectionService",
		Use:         "get-risk-objects",
		Short:       "SecurityProtectionService_GetRiskObjects",
		OperationID: "SecurityProtectionService_GetRiskObjects",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/security-protection/risk-objects",
		Params: []runtime.ParamSpec{
			{Name: "hours", Flag: "hours", In: "query", GoType: "int64", Help: "time window in hours (1-720, default 24) (query, int32)", Required: false, Format: "int32"},
			{Name: "cluster", Flag: "cluster", In: "query", GoType: "string", Help: "filter by cluster name (query)", Required: false},
			{Name: "level", Flag: "level", In: "query", GoType: "string", Help: "filter by risk level: high/medium/low (query)", Required: false},
		},
		Output: runtime.OutputHints{ListPath: "items", DefaultColumns: []string{"name", "color", "displayValue", "rank", "value"}},
	},
	{
		Group:       "SecurityProtectionService",
		Use:         "get-risk-types",
		Short:       "SecurityProtectionService_GetRiskTypes",
		OperationID: "SecurityProtectionService_GetRiskTypes",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/security-protection/risk-types",
		Params: []runtime.ParamSpec{
			{Name: "hours", Flag: "hours", In: "query", GoType: "int64", Help: "time window in hours (1-720, default 24) (query, int32)", Required: false, Format: "int32"},
			{Name: "cluster", Flag: "cluster", In: "query", GoType: "string", Help: "filter by cluster name (query)", Required: false},
			{Name: "level", Flag: "level", In: "query", GoType: "string", Help: "filter by risk level: high/medium/low (query)", Required: false},
		},
		Output: runtime.OutputHints{ListPath: "items", DefaultColumns: []string{"name", "color", "displayValue", "rank", "value"}},
	},
	{
		Group:       "SecurityProtectionService",
		Use:         "get-suggestions",
		Short:       "SecurityProtectionService_GetSuggestions",
		OperationID: "SecurityProtectionService_GetSuggestions",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/security-protection/suggestions",
		Params: []runtime.ParamSpec{
			{Name: "hours", Flag: "hours", In: "query", GoType: "int64", Help: "time window in hours (1-720, default 24) (query, int32)", Required: false, Format: "int32"},
			{Name: "cluster", Flag: "cluster", In: "query", GoType: "string", Help: "filter by cluster name (query)", Required: false},
			{Name: "level", Flag: "level", In: "query", GoType: "string", Help: "filter by risk level: high/medium/low (query)", Required: false},
		},
		Output: runtime.OutputHints{ListPath: "items", DefaultColumns: []string{"id", "desc", "icon", "priority", "title"}},
	},
	{
		Group:       "SecurityProtectionService",
		Use:         "get-timeline",
		Short:       "SecurityProtectionService_GetTimeline",
		OperationID: "SecurityProtectionService_GetTimeline",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/security-protection/timeline",
		Params: []runtime.ParamSpec{
			{Name: "hours", Flag: "hours", In: "query", GoType: "int64", Help: "hours (query, int32)", Required: false, Format: "int32"},
			{Name: "cluster", Flag: "cluster", In: "query", GoType: "string", Help: "cluster (query)", Required: false},
			{Name: "level", Flag: "level", In: "query", GoType: "string", Help: "level (query)", Required: false},
			{Name: "page", Flag: "page", In: "query", GoType: "int64", Help: "page number (default 1) (query, int32)", Required: false, Format: "int32"},
			{Name: "pageSize", Flag: "page-size", In: "query", GoType: "int64", Help: "items per page (default 10, max 100) (query, int32)", Required: false, Format: "int32"},
		},
		Output: runtime.OutputHints{ListPath: "items", DefaultColumns: []string{"level", "status", "text", "time"}, Pagination: &runtime.PaginationHint{Strategy: "offset", TokenParam: "page", LimitParam: "pageSize"}},
	},
	{
		Group:       "SinglePageService",
		Use:         "get-gain-comparison-rates",
		Short:       "SinglePageService_GetGainComparisonRates",
		OperationID: "SinglePageService_GetGainComparisonRates",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/singlepage/gain-comparison-rates",
		Output:      runtime.OutputHints{ListPath: "rates", DefaultColumns: []string{"gainRate", "metricId"}},
	},
	{
		Group:       "SinglePageService",
		Use:         "get-global-traffic",
		Short:       "SinglePageService_GetGlobalTraffic",
		OperationID: "SinglePageService_GetGlobalTraffic",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/singlepage/global-traffic",
		Output:      runtime.OutputHints{ListPath: "nodes", DefaultColumns: []string{"name", "color", "latitude", "longitude", "percentage", "primary"}},
	},
	{
		Group:       "SinglePageService",
		Use:         "get-gpu-resource-status",
		Short:       "SinglePageService_GetGpuResourceStatus",
		OperationID: "SinglePageService_GetGpuResourceStatus",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/singlepage/gpu-resource-status",
		Output:      runtime.OutputHints{ListPath: "deviceStates"},
	},
	{
		Group:       "SinglePageService",
		Use:         "get-popular-model-contribution",
		Short:       "SinglePageService_GetPopularModelContribution",
		OperationID: "SinglePageService_GetPopularModelContribution",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/singlepage/popular-model-contribution",
		Params: []runtime.ParamSpec{
			{Name: "timeWindow", Flag: "time-window", In: "query", GoType: "string", Help: "timeWindow (query)", Required: false},
			{Name: "cluster", Flag: "cluster", In: "query", GoType: "string", Help: "cluster (query)", Required: false},
			{Name: "limit", Flag: "limit", In: "query", GoType: "int64", Help: "limit (query, int32)", Required: false, Format: "int32"},
		},
		Output: runtime.OutputHints{ListPath: "items", DefaultColumns: []string{"modelName", "rank", "requestBarPercent", "requestCount", "requestDisplayValue", "requestRate"}, Pagination: &runtime.PaginationHint{Strategy: "cursor", LimitParam: "limit"}},
	},
	{
		Group:       "SinglePageService",
		Use:         "get-realtime-request-rate",
		Short:       "SinglePageService_GetRealtimeRequestRate",
		OperationID: "SinglePageService_GetRealtimeRequestRate",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/singlepage/realtime-request-rate",
	},
	{
		Group:       "SinglePageService",
		Use:         "get-revenue-per-minute",
		Short:       "SinglePageService_GetRevenuePerMinute",
		OperationID: "SinglePageService_GetRevenuePerMinute",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/singlepage/revenue-per-minute",
	},
	{
		Group:       "SinglePageService",
		Use:         "get-revenue-per-minute-history",
		Short:       "SinglePageService_GetRevenuePerMinuteHistory",
		OperationID: "SinglePageService_GetRevenuePerMinuteHistory",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/singlepage/revenue-per-minute-history",
		Params: []runtime.ParamSpec{
			{Name: "steps", Flag: "steps", In: "query", GoType: "int64", Help: "steps (query, int32)", Required: false, Format: "int32"},
		},
		Output: runtime.OutputHints{ListPath: "points", DefaultColumns: []string{"revenuePerMinute", "timestamp"}},
	},
	{
		Group:       "SinglePageService",
		Use:         "get-token-latency",
		Short:       "SinglePageService_GetTokenLatency",
		OperationID: "SinglePageService_GetTokenLatency",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/singlepage/token-latency",
		Params: []runtime.ParamSpec{
			{Name: "tzOffsetMinutes", Flag: "tz-offset-minutes", In: "query", GoType: "int64", Help: "Client timezone offset from UTC in minutes. Follows JavaScript (query, int32)", Required: false, Format: "int32"},
		},
	},
	{
		Group:       "SinglePageService",
		Use:         "get-usage-capacity-trend",
		Short:       "SinglePageService_GetUsageCapacityTrend",
		OperationID: "SinglePageService_GetUsageCapacityTrend",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/singlepage/usage-capacity-trend",
		Output:      runtime.OutputHints{ListPath: "actualPoints", DefaultColumns: []string{"date", "gpuUtilizationAvailable", "gpuUtilizationPct", "inputTokens", "inputTokensAvailable", "outputTokens"}},
	},
	{
		Group:       "UserMetricsService",
		Use:         "get-new-user-trend",
		Short:       "GetNewUserTrend returns recent daily or monthly new-user counts.",
		OperationID: "UserMetricsService_GetNewUserTrend",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/user-metrics/new-user-trend",
		Params: []runtime.ParamSpec{
			{Name: "range", Flag: "range", In: "query", GoType: "string", Help: "range (query, required, one of: NEW_USER_TREND_RANGE_UNSPECIFIED|NEW_USER_TREND_RANGE_LAST_30_DAYS|NEW_USER_TREND_RANGE_LAST_1_YEAR)", Required: true, Default: "NEW_USER_TREND_RANGE_UNSPECIFIED", Enum: []string{"NEW_USER_TREND_RANGE_UNSPECIFIED", "NEW_USER_TREND_RANGE_LAST_30_DAYS", "NEW_USER_TREND_RANGE_LAST_1_YEAR"}},
		},
		Output: runtime.OutputHints{ListPath: "points", DefaultColumns: []string{"bucketStart", "newUsersCount"}},
	},
	{
		Group:       "UserMetricsService",
		Use:         "get-registered-user-metrics",
		Short:       "GetRegisteredUserMetrics returns current and previous-period registration counts.",
		OperationID: "UserMetricsService_GetRegisteredUserMetrics",
		Method:      "GET",
		PathTpl:     "/apis/crane.io/v1alpha1/user-metrics/registered-users",
		Params: []runtime.ParamSpec{
			{Name: "timeRange", Flag: "time-range", In: "query", GoType: "string", Help: "Required UTC natural dashboard period. (query, required, one of: DASHBOARD_TIME_RANGE_UNSPECIFIED|DASHBOARD_TIME_RANGE_TODAY|DASHBOARD_TIME_RANGE_THIS_WEEK|DASHBOARD_TIME_RANGE_THIS_MONTH|DASHBOARD_TIME_RANGE_THIS_QUARTER)", Required: true, Default: "DASHBOARD_TIME_RANGE_UNSPECIFIED", Enum: []string{"DASHBOARD_TIME_RANGE_UNSPECIFIED", "DASHBOARD_TIME_RANGE_TODAY", "DASHBOARD_TIME_RANGE_THIS_WEEK", "DASHBOARD_TIME_RANGE_THIS_MONTH", "DASHBOARD_TIME_RANGE_THIS_QUARTER"}},
		},
	},
}
